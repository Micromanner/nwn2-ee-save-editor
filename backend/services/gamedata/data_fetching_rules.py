"""Data fetching rules to prevent infinite retry loops when managers can't find table data."""
...
class DataFetchingRules:
    """Manages data fetching rules with retry limits, cooldowns, and blacklisting."""
    
    def __init__(self, max_retries: int = 5, cooldown_seconds: int = 2, blacklist_after: int = 15):
        """Initialize with retry limits and cooldown settings."""
    ...
    def should_allow_request(self, table_name: str, method_name: str = "get_table", record_id: Optional[int] = None) -> bool:
        """Check if a data request should be allowed based on retry limits and cooldowns."""
        cache_key = self._get_cache_key(table_name, method_name, record_id)
    ...
    def record_failure(
        ...
        record_id: Optional[int] = None, error: Optional[Exception] = None
    ) -> None:
        ...
        with self._lock:
            ...
            logger.warning(f"Recorded failure: {cache_key} ({failure_count}/{self.blacklist_after}){error_msg}")

    def reset_table(self, table_name: str, method_name: str = "get_table", record_id: Optional[int] = None) -> None:
        """Reset all counters for a specific table."""
        cache_key = self._get_cache_key(table_name, method_name, record_id)
...
def get_data_fetching_rules() -> DataFetchingRules:
    """Get the global DataFetchingRules singleton instance."""
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(*args, **kwargs):
            ...
            if not rules.should_allow_request(table_name, actual_method_name, record_id):
                ...
                if is_initializing:
                    logger.info(
                        f"Data request during initialization: {actual_method_name}({table_name}"
                        f"{f', {record_id}' if record_id is not None else ''}) - system still loading"
                    )
                    return None
                elif is_blacklisted:
                    logger.warning(
                        f"Data request BLACKLISTED: {actual_method_name}({table_name}"
                        f"{f', {record_id}' if record_id is not None else ''}) - permanently blocked"
                    )
                else:
                    logger.warning(
                        f"Data request BLOCKED: {actual_method_name}({table_name}"
                        f"{f', {record_id}' if record_id is not None else ''}) - retry count: {retry_count}/{rules.max_retries}"
                    )
                return None
            ...
            if table_name == 'feat' and actual_method_name in ['get_table', 'get_by_id']:
                if not hasattr(rules, '_feat_access_started'):
                    ...
                    logger.warning(
                        f"First feat table access: {actual_method_name}({table_name}"
                        f"{f', {record_id}' if record_id is not None else ''})"
                    )
            ...
            try:
                ...
                if result is not None:
                    ...
                    # For get_by_id, None is valid (ID not found)
                    rules.record_success(table_name, actual_method_name, record_id)
                    cache_key = rules._get_cache_key(table_name, actual_method_name, record_id)
                    if cache_key in rules._retry_counts or cache_key in rules._failure_counts:
                        logger.info(
                            f"Data fetch resolved: get_by_id({table_name}, {record_id}) returned None (expected)"
                        )
                else:
                    logger.warning(
                        f"Data fetch FAILURE: {actual_method_name}({table_name}) returned None unexpectedly"
                    )
                ...
                logger.error(
                    f"Data fetch EXCEPTION: {actual_method_name}({table_name}"
                    f"{f', {record_id}' if record_id is not None else ''}) raised {type(e).__name__}: {e}"
                )
...
def clear_all_data_fetching_blocks() -> None:
    ...
    with rules._lock:
        ...
        logger.info(
            f"Cleared all data fetching blocks: {retry_count} retry, {failure_count} failure, {blacklist_count} blacklisted"
        )