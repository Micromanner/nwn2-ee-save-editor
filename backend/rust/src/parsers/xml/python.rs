use pyo3::prelude::*;
use super::parser::{RustXmlParser};
...
impl PyXmlParser {
    ...
    fn get_companion_status(&self, py: Python) -> PyResult<Py<PyAny>> {
        let status = self.inner.get_companion_status();
        Ok(pythonize(py, &status)?.into())
    }

    fn get_quest_overview(&self, py: Python) -> PyResult<Py<PyAny>> {
        let overview = self.inner.get_quest_overview_struct();
        Ok(pythonize(py, &overview)?.into())
    }

    fn get_general_info(&self, py: Python) -> PyResult<Py<PyAny>> {
        let info = self.inner.get_general_info();
        Ok(pythonize(py, &info)?.into())
    }

    fn get_full_summary(&self, py: Python) -> PyResult<Py<PyAny>> {
        let summary = self.inner.get_full_summary_struct();
        Ok(pythonize(py, &summary)?.into())
    }

    // Variable Accessors
    fn get_all_integers(&self, py: Python) -> PyResult<Py<PyAny>> {
        Ok(pythonize(py, &self.inner.data.integers)?.into())
    }

    fn get_all_strings(&self, py: Python) -> PyResult<Py<PyAny>> {
        Ok(pythonize(py, &self.inner.data.strings)?.into())
    }

    fn get_all_floats(&self, py: Python) -> PyResult<Py<PyAny>> {
        Ok(pythonize(py, &self.inner.data.floats)?.into())
    }
    
    fn get_variable(&self, py: Python, var_name: &str, var_type: &str) -> PyResult<Py<PyAny>> {
        use pyo3::IntoPyObjectExt;
        if var_type == "int" {
             if let Some(val) = self.inner.data.integers.get(var_name) {
                 return Ok(val.into_py_any(py)?.into_any());
             }
        } else if var_type == "string" {
             if let Some(val) = self.inner.data.strings.get(var_name) {
                 return Ok(val.into_py_any(py)?.into_any());
             }
        } else if var_type == "float" {
             if let Some(val) = self.inner.data.floats.get(var_name) {
                 return Ok(val.into_py_any(py)?.into_any());
             }
        }
        Ok(py.None())
    ...
    fn update_companion_influence(&mut self, companion_id: &str, new_influence: i32) -> bool {
        let definitions = super::parser::get_companion_definitions();
