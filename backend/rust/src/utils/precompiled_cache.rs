...
impl CacheBuilder {
    ...
    fn build_cache(&self,
        ...
        for (key, value) in tables_data.iter() {
            ...
            let table_info: Bound<'_, PyDict> = value.extract()?;
    ...
    fn generate_cache_key(&self, _py: Python, mod_state: &Bound<'_, PyDict>) -> PyResult<String> {
        ...
        if let Ok(Some(workshop_files)) = mod_state.get_item("workshop_files") {
            if let Ok(files) = workshop_files.extract::<Bound<'_, PyList>>() {
        ...
        if let Ok(Some(override_files)) = mod_state.get_item("override_files") {
            if let Ok(files) = override_files.extract::<Bound<'_, PyList>>() {
                let mut sorted_files: Vec<String> = files.extract()?;
                sorted_files.sort();
