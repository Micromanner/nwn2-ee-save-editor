"""Pydantic models for ClassManager operations."""
...
class ClassInfo(BaseModel):
    """Complete class information matching ClassCategorizer output."""
...
class ClassLevel(BaseModel):
    """Individual class level information."""
...
class MulticlassInfo(BaseModel):
    """Multiclassing status information."""
...
class CombatProgression(BaseModel):
    """Combat statistics including BAB and saves."""
...
class ClassSummary(BaseModel):
    """Complete class summary including levels and multiclass status."""
...
class ClassState(BaseModel):
    """Complete class state including available options."""
...
class ClassChangeRequest(BaseModel):
    ...
    old_class_id: Optional[int] = Field(None, description="Old class ID for replacement")
...
class ClassProgressionPreview(BaseModel):
    """Preview of class progression features."""
    class_id: int
    class_name: str
...
class LevelUpRequest(BaseModel):
    ...
    level_change: int = Field(1, description="Number of levels to add/remove")
...
class ClassAddRequest(BaseModel):
    """Request to add a new class (multiclass)."""
    class_id: int = Field(..., description="Class ID to add")