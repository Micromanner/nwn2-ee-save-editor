"""Combat manager for AC calculation, initiative, and combat statistics."""
...
class CombatManager(EventEmitter):
    """Handles AC calculation, initiative, and combat statistics."""
    ...
    def calculate_initiative(self) -> Dict[str, Any]:
        """Calculate initiative modifier and return breakdown."""
    ...
    def calculate_combat_maneuver_bonus(self) -> Dict[str, Any]:
        """Calculate CMB for grapple, trip, etc (D&D 3.5/Pathfinder style)."""
    ...
    def _get_equipped_item(self, slot: str) -> Optional[Any]:
        """Get item equipped in specific slot using inventory_manager's slot mapping."""
        old_to_new_slot_map = {
            'Head': 'head',
            'Chest': 'chest',
            'Boots': 'boots',
    ...
    def _get_item_ac_bonus(self, item) -> int:
        ...
        # Use ArmorRulesType if AC field not available
    ...
    def _is_shield(self, item) -> bool:
        ...
        # Check Category field - shields have Category=3 in baseitems.2da
        category = self.field_mapper.get_field_value(base_item_data, 'category', None)
        ...
        # Check item_class for 'shield' string (handles modded content)
    ...
    def _is_heavy_armor(self, chest_item) -> bool:
        """Check if armor type reduces movement speed by looking up RANK in armorrulestats.2da."""
        if not chest_item:
            return False
        
        armor_rules_type = chest_item.get('ArmorRulesType', None)
        if armor_rules_type is None:
            return False
        
        armor_stats = self.rules_service.get_table('armorrulestats')
        if not armor_stats or armor_rules_type >= len(armor_stats):
            return False
        
        armor_row = armor_stats[armor_rules_type]
        rank = self.field_mapper.get_field_value(armor_row, 'rank', 'None')
        return rank == 'Heavy'
    ...
    def _get_movement_speed(self) -> Dict[str, int]:
        """Get movement speed affected by armor and class features."""
        race_manager = self.character_manager.get_manager('race')
        if race_manager:
            race_id = self.gff.get('Race', 6)
            base_speed = race_manager.get_base_speed(race_id)
        else:
            base_speed = 30
        
        chest_item = self._get_equipped_item('Chest')
        if chest_item and self._is_heavy_armor(chest_item):
            base_speed = 20
    ...
    def get_attack_bonuses(self) -> Dict[str, Any]:
        """Get melee and ranged attack bonuses with component breakdown."""
    ...
    def calculate_base_attack_bonus(self) -> int:
        ...
        # Use level history if complete, otherwise fallback to ClassList summation
        if lvl_stat_list and isinstance(lvl_stat_list, list) and len(lvl_stat_list) == total_char_level:
            ...
            for i, level_entry in enumerate(lvl_stat_list):
                ...
                if char_level <= 20:
                    ...
                    # Epic: +1 BAB at odd character levels (21, 23, 25...)
                    if char_level % 2 != 0:
                        total_bab += 1
        else:
            class_list = self.gff.get('ClassList', [])
    ...
    def _calculate_class_bab(self, class_data, level: int) -> int:
        """Calculate BAB for a single class at a specific level."""
        if level <= 0:
            return 0
        ...
        # Tables are 0-indexed, cap at level 20
    ...
    def update_natural_armor(self, value: int) -> Dict[str, Any]:
        """Set natural armor bonus with engine limit validation (0-255)."""
    ...
    def update_initiative_bonus(self, value: int) -> Dict[str, Any]:
        """Set initiative misc bonus with engine limit validation (-128 to +127)."""
    ...
    def get_attack_sequence(self) -> List[int]:
        """Get attack sequence based on BAB."""
        bab = self.calculate_base_attack_bonus()
        attacks = [bab]
        while bab > 5:
            bab -= 5
    ...
    def get_spell_resistance(self) -> int:
        """Get spell resistance value from race and other sources."""
    ...
    def update_hit_points(self, current: Optional[int] = None, max_hp: Optional[int] = None) -> Dict[str, int]:
        """Update current and/or max hit points."""
        updates = {}
