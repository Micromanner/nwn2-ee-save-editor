use crate::error::ErfError;
use crate::parser::ErfParser;
use crate::types::extension_to_resource_type;
...
impl PyErfParser {
    ...
    fn get_statistics(&self, py: Python) -> PyResult<Py<PyDict>> {
        ...
        if let Some((name, size)) = &stats.largest_resource {
            let largest = PyDict::new_bound(py);
    ...
    fn get_version(&self) -> Option<String> {
        self.inner.version.map(|v| match v {
            crate::types::ErfVersion::V10 => "V1.0",
            crate::types::ErfVersion::V11 => "V1.1",
        }.to_string())
    }
