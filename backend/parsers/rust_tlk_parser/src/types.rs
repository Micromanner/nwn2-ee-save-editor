...
use string_interner::{DefaultSymbol, StringInterner, DefaultBackend, Symbol};
...
pub struct TLKParser {
    ...
    /// Security limits
    pub security_limits: crate::error::SecurityLimits,
    /// Parser statistics
...
impl TLKParser {
    ...
    pub fn new() -> Self {
        Self {
            ...
            string_data: Vec::new(),
            security_limits: crate::error::SecurityLimits::default(),
    ...
    pub fn with_limits(limits: crate::error::SecurityLimits) -> Self {
        Self {
    ...
    pub fn security_limits_mut(&mut self) -> &mut crate::error::SecurityLimits {
    ...
    pub fn to_serializable(&self) -> SerializableTLKParser {
        ...
        let interner_data: Vec<String> = self.interner.into_iter()
            .map(|(_, s)| s.to_string())
            .collect();
        ...
        for (index, cached) in &self.string_cache {
            string_mappings.insert(*index, cached.symbol.to_usize());
        }
