...
impl PyRustIconCache {
    ...
    pub fn initialize<'py>(&self, py: Python<'py>, force_reload: bool) -> PyResult<&'py PyAny> {
    ...
    pub fn get_icon(&self, name: &str) -> Option<(Py<PyBytes>, String)> {
    ...
    pub fn get_icon_async<'py>(&self, py: Python<'py>, name: String) -> PyResult<&'py PyAny> {
        let inner = self.inner.clone();
        ...
        future_into_py(py, async move {
            ...
            match icon {
                Some(icon) => {
                    Python::with_gil(|py| {
                        let bytes = PyBytes::new(py, &icon.data);
                        let result: (Py<PyBytes>, String) = (bytes.into_py(py), icon.format.mime_type().to_string());
                        Ok(Some(result))
                    })
                }
                None => Ok(None),
    ...
    pub fn get_icons_batch(&self, names: Vec<String>) -> Vec<Option<(Py<PyBytes>, String)>> {
    ...
    /// Set module HAKs
    pub fn set_module_haks<'py>(&self, py: Python<'py>, hak_list: Vec<String>) -> PyResult<&'py PyAny> {
        let inner = self.inner.clone();
    ...
    /// Load module icons
    pub fn load_module_icons<'py>(&self, py: Python<'py>, module_path: String) -> PyResult<&'py PyAny> {
        let inner = self.inner.clone();
...
#[pymodule]
fn rust_icon_cache(_py: Python, m: &PyModule) -> PyResult<()> {
    // Initialize logging for Rust - this will honor RUST_LOG env var
