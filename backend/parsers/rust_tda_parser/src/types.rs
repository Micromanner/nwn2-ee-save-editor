...
pub type TDAStringInterner = StringInterner<string_interner::DefaultBackend>;
...
impl CellValue {
    ...
    pub fn as_str<'a>(&'a self, interner: &'a TDAStringInterner) -> Option<&'a str> {
        match self {
            Self::Interned(symbol) => interner.resolve(*symbol),
...
impl TDAParser {
    ...
    pub fn with_limits(limits: SecurityLimits) -> Self {
        Self {
            interner: TDAStringInterner::new(),
            columns: Vec::new(),
    ...
    pub fn column_names(&self) -> Vec<&str> {
        self.columns
            .iter()
            .filter_map(|col| self.interner.resolve(col.name))
            .collect()
    }
    ...
    pub fn get_row_dict(&self, row_index: usize) -> TDAResult<AHashMap<String, Option<String>>> {
        ...
        for (col_info, cell) in self.columns.iter().zip(row.iter()) {
            if let Some(col_name) = self.interner.resolve(col_info.name) {
    ...
    pub fn clear(&mut self) {
        ...
        self.interner = TDAStringInterner::new();
        self.columns.clear();
...
impl SerializableTDAParser {
    ...
    pub fn from_parser(parser: &TDAParser) -> Self {
            ...
            row.iter().map(|cell| {
                match cell {
                    CellValue::Interned(symbol) => {
                        if let Some(s) = parser.interner().resolve(*symbol) {
                            SerializableCellValue::String(s.to_string())
                        } else {
                            SerializableCellValue::Null
                        }
                    }
        ...
        Self {
            column_names,
            rows,
            security_limits: parser.security_limits().clone(),
