...
class SurfacematData:
    """
    Auto-generated data class for surfacemat.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label_Walk_WalkCheck_LineOfSight_Sound_Name', '_IsWater', '_col_empty', '_Visual')
    _column_mapping = {'Label           Walk   WalkCheck   LineOfSight   Sound   Name': 'Label_Walk_WalkCheck_LineOfSight_Sound_Name', 'IsWater': 'IsWater', '': 'col_empty', 'Visual': 'Visual'}
    _table_name = 'surfacemat'
    _safe_columns = ['Label_Walk_WalkCheck_LineOfSight_Sound_Name', 'IsWater', 'col_empty', 'Visual']
    ...
    def __repr__(self):
        ...
        pk_column = 'Label_Walk_WalkCheck_LineOfSight_Sound_Name'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<SurfacematData {pk_column}={pk_value!r}>>"
        else:
            return f"<SurfacematData>"
