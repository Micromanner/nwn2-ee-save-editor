...
class Skill_mountedData:
    """
    Auto-generated data class for skill_mounted.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Name', '_Description', '_Critter', '_RideRanks', '_Speed', '_Heartiness')
    _column_mapping = {'Label': 'Label', 'Name': 'Name', 'Description': 'Description', 'Critter': 'Critter', 'RideRanks': 'RideRanks', 'Speed': 'Speed', 'Heartiness': 'Heartiness'}
    _table_name = 'skill_mounted'
    _safe_columns = ['Label', 'Name', 'Description', 'Critter', 'RideRanks', 'Speed', 'Heartiness']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Skill_mountedData {pk_column}={pk_value!r}>>"
        else:
            return f"<Skill_mountedData>"
