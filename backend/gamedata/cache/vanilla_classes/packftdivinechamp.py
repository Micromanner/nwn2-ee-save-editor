...
class PackftdivinechampData:
    """
    Auto-generated data class for packftdivinechamp.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_FeatIndex', '_Label')
    _column_mapping = {'FeatIndex': 'FeatIndex', 'Label': 'Label'}
    _table_name = 'packftdivinechamp'
    _safe_columns = ['FeatIndex', 'Label']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<PackftdivinechampData {pk_column}={pk_value!r}>>"
        else:
            return f"<PackftdivinechampData>"
