...
class LoadscreensData:
    """
    Auto-generated data class for loadscreens.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_ScriptingName', '_BMPResRef', '_TileSet', '_StrRef')
    _column_mapping = {'Label': 'Label', 'ScriptingName': 'ScriptingName', 'BMPResRef': 'BMPResRef', 'TileSet': 'TileSet', 'StrRef': 'StrRef'}
    _table_name = 'loadscreens'
    _safe_columns = ['Label', 'ScriptingName', 'BMPResRef', 'TileSet', 'StrRef']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<LoadscreensData {pk_column}={pk_value!r}>>"
        else:
            return f"<LoadscreensData>"
