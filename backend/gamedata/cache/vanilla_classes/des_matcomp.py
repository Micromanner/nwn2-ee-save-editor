...
class Des_matcompData:
    """
    Auto-generated data class for des_matcomp.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_InnateLvl', '_comp_tag')
    _column_mapping = {'Label': 'Label', 'InnateLvl': 'InnateLvl', 'comp_tag': 'comp_tag'}
    _table_name = 'des_matcomp'
    _safe_columns = ['Label', 'InnateLvl', 'comp_tag']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_matcompData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_matcompData>"
