...
class SpelltargetData:
    """
    Auto-generated data class for spelltarget.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Shape', '_Width', '_Length', '_Image', '_ImageHostile')
    _column_mapping = {'Label': 'Label', 'Shape': 'Shape', 'Width': 'Width', 'Length': 'Length', 'Image': 'Image', 'ImageHostile': 'ImageHostile'}
    _table_name = 'spelltarget'
    _safe_columns = ['Label', 'Shape', 'Width', 'Length', 'Image', 'ImageHostile']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SpelltargetData {pk_column}={pk_value!r}>>"
        else:
            return f"<SpelltargetData>"
