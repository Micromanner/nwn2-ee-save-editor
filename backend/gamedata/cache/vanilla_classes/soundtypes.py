...
class SoundtypesData:
    """
    Auto-generated data class for soundtypes.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_TemplateResRef')
    _column_mapping = {'Label': 'Label', 'TemplateResRef': 'TemplateResRef'}
    _table_name = 'soundtypes'
    _safe_columns = ['Label', 'TemplateResRef']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SoundtypesData {pk_column}={pk_value!r}>>"
        else:
            return f"<SoundtypesData>"
