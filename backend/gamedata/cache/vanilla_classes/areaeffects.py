...
class AreaeffectsData:
    """
    Auto-generated data class for areaeffects.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_ID', '_SHAPE', '_RADIUS', '_WIDTH', '_LENGTH', '_ONENTER', '_HEARTBEAT', '_ONEXIT')
    _column_mapping = {'LABEL': 'LABEL', 'ID': 'ID', 'SHAPE': 'SHAPE', 'RADIUS': 'RADIUS', 'WIDTH': 'WIDTH', 'LENGTH': 'LENGTH', 'ONENTER': 'ONENTER', 'HEARTBEAT': 'HEARTBEAT', 'ONEXIT': 'ONEXIT'}
    _table_name = 'areaeffects'
    _safe_columns = ['LABEL', 'ID', 'SHAPE', 'RADIUS', 'WIDTH', 'LENGTH', 'ONENTER', 'HEARTBEAT', 'ONEXIT']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<AreaeffectsData {pk_column}={pk_value!r}>>"
        else:
            return f"<AreaeffectsData>"
