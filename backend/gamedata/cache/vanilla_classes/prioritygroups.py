...
class PrioritygroupsData:
    """
    Auto-generated data class for prioritygroups.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Priority', '_Volume', '_MaxPlaying', '_Interrupt', '_FadeTime', '_MaxVolumeDist', '_MinVolumeDist', '_PlaybackVariance')
    _column_mapping = {'Label': 'Label', 'Priority': 'Priority', 'Volume': 'Volume', 'MaxPlaying': 'MaxPlaying', 'Interrupt': 'Interrupt', 'FadeTime': 'FadeTime', 'MaxVolumeDist': 'MaxVolumeDist', 'MinVolumeDist': 'MinVolumeDist', 'PlaybackVariance': 'PlaybackVariance'}
    _table_name = 'prioritygroups'
    _safe_columns = ['Label', 'Priority', 'Volume', 'MaxPlaying', 'Interrupt', 'FadeTime', 'MaxVolumeDist', 'MinVolumeDist', 'PlaybackVariance']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<PrioritygroupsData {pk_column}={pk_value!r}>>"
        else:
            return f"<PrioritygroupsData>"
