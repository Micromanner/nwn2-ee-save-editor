...
class ItempropdefData:
    """
    Auto-generated data class for itempropdef.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_SubTypeResRef', '_Cost', '_CostTableResRef', '_Param1ResRef', '_GameStrRef', '_Description', '_Slots', '_col_empty')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'SubTypeResRef': 'SubTypeResRef', 'Cost': 'Cost', 'CostTableResRef': 'CostTableResRef', 'Param1ResRef': 'Param1ResRef', 'GameStrRef': 'GameStrRef', 'Description': 'Description', 'Slots': 'Slots', '': 'col_empty'}
    _table_name = 'itempropdef'
    _safe_columns = ['Name', 'Label', 'SubTypeResRef', 'Cost', 'CostTableResRef', 'Param1ResRef', 'GameStrRef', 'Description', 'Slots', 'col_empty']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ItempropdefData {pk_column}={pk_value!r}>>"
        else:
            return f"<ItempropdefData>"
