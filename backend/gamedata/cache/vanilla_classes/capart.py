...
class CapartData:
    """
    Auto-generated data class for capart.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_NAME', '_MDLNAME', '_NODENAME')
    _column_mapping = {'NAME': 'NAME', 'MDLNAME': 'MDLNAME', 'NODENAME': 'NODENAME'}
    _table_name = 'capart'
    _safe_columns = ['NAME', 'MDLNAME', 'NODENAME']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<CapartData {pk_column}={pk_value!r}>>"
        else:
            return f"<CapartData>"
