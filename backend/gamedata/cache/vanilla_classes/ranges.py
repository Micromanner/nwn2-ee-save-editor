...
class RangesData:
    """
    Auto-generated data class for ranges.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_PrimaryRange', '_SecondaryRange', '_Name')
    _column_mapping = {'Label': 'Label', 'PrimaryRange': 'PrimaryRange', 'SecondaryRange': 'SecondaryRange', 'Name': 'Name'}
    _table_name = 'ranges'
    _safe_columns = ['Label', 'PrimaryRange', 'SecondaryRange', 'Name']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<RangesData {pk_column}={pk_value!r}>>"
        else:
            return f"<RangesData>"
