...
class Cls_featmap_sacredfistData:
    """
    Auto-generated data class for cls_featmap_sacredfist.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_ClassId', '_ClassName', '_SpellcasterFeat')
    _column_mapping = {'ClassId': 'ClassId', 'ClassName': 'ClassName', 'SpellcasterFeat': 'SpellcasterFeat'}
    _table_name = 'cls_featmap_sacredfist'
    _safe_columns = ['ClassId', 'ClassName', 'SpellcasterFeat']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Cls_featmap_sacredfistData {pk_column}={pk_value!r}>>"
        else:
            return f"<Cls_featmap_sacredfistData>"
