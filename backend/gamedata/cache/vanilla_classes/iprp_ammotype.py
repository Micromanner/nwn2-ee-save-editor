...
class Iprp_ammotypeData:
    """
    Auto-generated data class for iprp_ammotype.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_AmmoType')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'AmmoType': 'AmmoType'}
    _table_name = 'iprp_ammotype'
    _safe_columns = ['Name', 'Label', 'AmmoType']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_ammotypeData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_ammotypeData>"
