...
class CrtemplatesData:
    """
    Auto-generated data class for crtemplates.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_NAME', '_STRREF')
    _column_mapping = {'NAME': 'NAME', 'STRREF': 'STRREF'}
    _table_name = 'crtemplates'
    _safe_columns = ['NAME', 'STRREF']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<CrtemplatesData {pk_column}={pk_value!r}>>"
        else:
            return f"<CrtemplatesData>"
