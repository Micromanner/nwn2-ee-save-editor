...
class Iprp_chargecostData:
    """
    Auto-generated data class for iprp_chargecost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Cost', '_PotionCost', '_WandCost')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Cost': 'Cost', 'PotionCost': 'PotionCost', 'WandCost': 'WandCost'}
    _table_name = 'iprp_chargecost'
    _safe_columns = ['Name', 'Label', 'Cost', 'PotionCost', 'WandCost']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_chargecostData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_chargecostData>"
