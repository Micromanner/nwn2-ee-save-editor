...
class Iprp_spellshlData:
    """
    Auto-generated data class for iprp_spellshl.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name_Label_Cost', '_Letter')
    _column_mapping = {'Name         Label           Cost': 'Name_Label_Cost', 'Letter': 'Letter'}
    _table_name = 'iprp_spellshl'
    _safe_columns = ['Name_Label_Cost', 'Letter']
    ...
    def __repr__(self):
        ...
        pk_column = 'Name_Label_Cost'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<Iprp_spellshlData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_spellshlData>"
