...
class ItmwizstavesData:
    """
    Auto-generated data class for itmwizstaves.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Prop', '_SubProp', '_CostTable', '_CostVal', '_ParamTable', '_Param', '_Value')
    _column_mapping = {'Prop': 'Prop', 'SubProp': 'SubProp', 'CostTable': 'CostTable', 'CostVal': 'CostVal', 'ParamTable': 'ParamTable', 'Param': 'Param', 'Value': 'Value'}
    _table_name = 'itmwizstaves'
    _safe_columns = ['Prop', 'SubProp', 'CostTable', 'CostVal', 'ParamTable', 'Param', 'Value']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ItmwizstavesData {pk_column}={pk_value!r}>>"
        else:
            return f"<ItmwizstavesData>"
