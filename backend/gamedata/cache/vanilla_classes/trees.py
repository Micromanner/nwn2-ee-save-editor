...
class TreesData:
    """
    Auto-generated data class for trees.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Stringref', '_Model')
    _column_mapping = {'Label': 'Label', 'Stringref': 'Stringref', 'Model': 'Model'}
    _table_name = 'trees'
    _safe_columns = ['Label', 'Stringref', 'Model']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<TreesData {pk_column}={pk_value!r}>>"
        else:
            return f"<TreesData>"
