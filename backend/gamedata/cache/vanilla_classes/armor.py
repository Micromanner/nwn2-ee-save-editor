...
class ArmorData:
    """
    Auto-generated data class for armor.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_ACBONUS', '_DEXBONUS', '_ACCHECK', '_ARCANEFAILUREpct', '_WEIGHT', '_COST', '_DESCRIPTIONS', '_BASEITEMSTATREF', '_Label', '_Name', '_ToolsetName', '_Prefix', '_Parent')
    _column_mapping = {'ACBONUS': 'ACBONUS', 'DEXBONUS': 'DEXBONUS', 'ACCHECK': 'ACCHECK', 'ARCANEFAILURE%': 'ARCANEFAILUREpct', 'WEIGHT': 'WEIGHT', 'COST': 'COST', 'DESCRIPTIONS': 'DESCRIPTIONS', 'BASEITEMSTATREF': 'BASEITEMSTATREF', 'Label': 'Label', 'Name': 'Name', 'ToolsetName': 'ToolsetName', 'Prefix': 'Prefix', 'Parent': 'Parent'}
    _table_name = 'armor'
    _safe_columns = ['ACBONUS', 'DEXBONUS', 'ACCHECK', 'ARCANEFAILUREpct', 'WEIGHT', 'COST', 'DESCRIPTIONS', 'BASEITEMSTATREF', 'Label', 'Name', 'ToolsetName', 'Prefix', 'Parent']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ArmorData {pk_column}={pk_value!r}>>"
        else:
            return f"<ArmorData>"
