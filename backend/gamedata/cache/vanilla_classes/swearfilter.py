...
class SwearfilterData:
    """
    Auto-generated data class for swearfilter.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_WORD', '_REPLACE', '_STANDALONE', '_EXCEPTIONS')
    _column_mapping = {'WORD': 'WORD', 'REPLACE': 'REPLACE', 'STANDALONE': 'STANDALONE', 'EXCEPTIONS': 'EXCEPTIONS'}
    _table_name = 'swearfilter'
    _safe_columns = ['WORD', 'REPLACE', 'STANDALONE', 'EXCEPTIONS']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SwearfilterData {pk_column}={pk_value!r}>>"
        else:
            return f"<SwearfilterData>"
