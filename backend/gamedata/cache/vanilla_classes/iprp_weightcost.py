...
class Iprp_weightcostData:
    """
    Auto-generated data class for iprp_weightcost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Cost', '_Value')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Cost': 'Cost', 'Value': 'Value'}
    _table_name = 'iprp_weightcost'
    _safe_columns = ['Name', 'Label', 'Cost', 'Value']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_weightcostData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_weightcostData>"
