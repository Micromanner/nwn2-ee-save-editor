...
class Game_paramsData:
    """
    Auto-generated data class for game_params.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Debug_Level')
    _column_mapping = {'Debug_Level': 'Debug_Level'}
    _table_name = 'game_params'
    _safe_columns = ['Debug_Level']
    ...
    def __repr__(self):
        ...
        pk_column = 'Debug_Level'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<Game_paramsData {pk_column}={pk_value!r}>>"
        else:
            return f"<Game_paramsData>"
