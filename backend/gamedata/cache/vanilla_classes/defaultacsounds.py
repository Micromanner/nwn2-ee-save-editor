...
class DefaultacsoundsData:
    """
    Auto-generated data class for defaultacsounds.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_ArmorType')
    _column_mapping = {'Label': 'Label', 'ArmorType': 'ArmorType'}
    _table_name = 'defaultacsounds'
    _safe_columns = ['Label', 'ArmorType']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<DefaultacsoundsData {pk_column}={pk_value!r}>>"
        else:
            return f"<DefaultacsoundsData>"
