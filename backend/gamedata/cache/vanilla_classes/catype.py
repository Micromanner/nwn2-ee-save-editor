...
class CatypeData:
    """
    Auto-generated data class for catype.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_NAME', '_TYPE_')
    _column_mapping = {'NAME': 'NAME', 'TYPE': 'TYPE_'}
    _table_name = 'catype'
    _safe_columns = ['NAME', 'TYPE_']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<CatypeData {pk_column}={pk_value!r}>>"
        else:
            return f"<CatypeData>"
