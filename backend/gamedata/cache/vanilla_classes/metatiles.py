...
class MetatilesData:
    """
    Auto-generated data class for metatiles.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_STRING_REF', '_TILESET', '_NAME', '_X', '_Y')
    _column_mapping = {'LABEL': 'LABEL', 'STRING_REF': 'STRING_REF', 'TILESET': 'TILESET', 'NAME': 'NAME', 'X': 'X', 'Y': 'Y'}
    _table_name = 'metatiles'
    _safe_columns = ['LABEL', 'STRING_REF', 'TILESET', 'NAME', 'X', 'Y']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<MetatilesData {pk_column}={pk_value!r}>>"
        else:
            return f"<MetatilesData>"
