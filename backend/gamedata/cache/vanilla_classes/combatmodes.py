...
class CombatmodesData:
    """
    Auto-generated data class for combatmodes.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_CombatMode_ActivatedName_DeactivatedName_NeedsTarget')
    _column_mapping = {'CombatMode                   ActivatedName   DeactivatedName   NeedsTarget': 'CombatMode_ActivatedName_DeactivatedName_NeedsTarget'}
    _table_name = 'combatmodes'
    _safe_columns = ['CombatMode_ActivatedName_DeactivatedName_NeedsTarget']
    ...
    def __repr__(self):
        ...
        pk_column = 'CombatMode_ActivatedName_DeactivatedName_NeedsTarget'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<CombatmodesData {pk_column}={pk_value!r}>>"
        else:
            return f"<CombatmodesData>"
