...
class PlaceableobjsndsData:
    """
    Auto-generated data class for placeableobjsnds.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_ArmorType', '_Opened', '_Closed', '_Destroyed', '_Used', '_Locked')
    _column_mapping = {'Label': 'Label', 'ArmorType': 'ArmorType', 'Opened': 'Opened', 'Closed': 'Closed', 'Destroyed': 'Destroyed', 'Used': 'Used', 'Locked': 'Locked'}
    _table_name = 'placeableobjsnds'
    _safe_columns = ['Label', 'ArmorType', 'Opened', 'Closed', 'Destroyed', 'Used', 'Locked']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<PlaceableobjsndsData {pk_column}={pk_value!r}>>"
        else:
            return f"<PlaceableobjsndsData>"
