...
class XpbaseconstData:
    """
    Auto-generated data class for xpbaseconst.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LevelDiff', '_Balance', '_Bonus')
    _column_mapping = {'LevelDiff': 'LevelDiff', 'Balance': 'Balance', 'Bonus': 'Bonus'}
    _table_name = 'xpbaseconst'
    _safe_columns = ['LevelDiff', 'Balance', 'Bonus']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<XpbaseconstData {pk_column}={pk_value!r}>>"
        else:
            return f"<XpbaseconstData>"
