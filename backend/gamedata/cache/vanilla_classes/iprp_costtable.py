...
class Iprp_costtableData:
    """
    Auto-generated data class for iprp_costtable.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_ClientLoad')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'ClientLoad': 'ClientLoad'}
    _table_name = 'iprp_costtable'
    _safe_columns = ['Name', 'Label', 'ClientLoad']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_costtableData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_costtableData>"
