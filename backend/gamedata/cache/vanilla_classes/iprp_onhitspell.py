...
class Iprp_onhitspellData:
    """
    Auto-generated data class for iprp_onhitspell.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Name', '_SpellIndex', '_Cost')
    _column_mapping = {'Label': 'Label', 'Name': 'Name', 'SpellIndex': 'SpellIndex', 'Cost': 'Cost'}
    _table_name = 'iprp_onhitspell'
    _safe_columns = ['Label', 'Name', 'SpellIndex', 'Cost']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_onhitspellData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_onhitspellData>"
