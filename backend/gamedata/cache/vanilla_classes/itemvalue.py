...
class ItemvalueData:
    """
    Auto-generated data class for itemvalue.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_DESIREDTREASURE', '_MAXSINGLEITEMVALUE', '_TOTALVALUEFILTER')
    _column_mapping = {'LABEL': 'LABEL', 'DESIREDTREASURE': 'DESIREDTREASURE', 'MAXSINGLEITEMVALUE': 'MAXSINGLEITEMVALUE', 'TOTALVALUEFILTER': 'TOTALVALUEFILTER'}
    _table_name = 'itemvalue'
    _safe_columns = ['LABEL', 'DESIREDTREASURE', 'MAXSINGLEITEMVALUE', 'TOTALVALUEFILTER']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ItemvalueData {pk_column}={pk_value!r}>>"
        else:
            return f"<ItemvalueData>"
