...
class Crafting_indexData:
    """
    Auto-generated data class for crafting_index.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_CATEGORY', '_START_ROW')
    _column_mapping = {'CATEGORY': 'CATEGORY', 'START_ROW': 'START_ROW'}
    _table_name = 'crafting_index'
    _safe_columns = ['CATEGORY', 'START_ROW']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Crafting_indexData {pk_column}={pk_value!r}>>"
        else:
            return f"<Crafting_indexData>"
