...
class PregenData:
    """
    Auto-generated data class for pregen.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_FILENAME')
    _column_mapping = {'LABEL': 'LABEL', 'FILENAME': 'FILENAME'}
    _table_name = 'pregen'
    _safe_columns = ['LABEL', 'FILENAME']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<PregenData {pk_column}={pk_value!r}>>"
        else:
            return f"<PregenData>"
