...
class Rrf_wavData:
    """
    Auto-generated data class for rrf_wav.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Filter', '_StringRef')
    _column_mapping = {'Label': 'Label', 'Filter': 'Filter', 'StringRef': 'StringRef'}
    _table_name = 'rrf_wav'
    _safe_columns = ['Label', 'Filter', 'StringRef']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Rrf_wavData {pk_column}={pk_value!r}>>"
        else:
            return f"<Rrf_wavData>"
