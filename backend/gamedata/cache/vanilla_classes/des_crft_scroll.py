...
class Des_crft_scrollData:
    """
    Auto-generated data class for des_crft_scroll.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Wiz_Sorc', '_Cleric', '_Paladin', '_Druid', '_Ranger', '_Bard')
    _column_mapping = {'Label': 'Label', 'Wiz_Sorc': 'Wiz_Sorc', 'Cleric': 'Cleric', 'Paladin': 'Paladin', 'Druid': 'Druid', 'Ranger': 'Ranger', 'Bard': 'Bard'}
    _table_name = 'des_crft_scroll'
    _safe_columns = ['Label', 'Wiz_Sorc', 'Cleric', 'Paladin', 'Druid', 'Ranger', 'Bard']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_crft_scrollData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_crft_scrollData>"
