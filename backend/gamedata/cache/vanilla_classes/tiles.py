...
class TilesData:
    """
    Auto-generated data class for tiles.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_STRING_REF', '_TILESET', '_TILE_TYPE', '_VARIATIONS')
    _column_mapping = {'LABEL': 'LABEL', 'STRING_REF': 'STRING_REF', 'TILESET': 'TILESET', 'TILE_TYPE': 'TILE_TYPE', 'VARIATIONS': 'VARIATIONS'}
    _table_name = 'tiles'
    _safe_columns = ['LABEL', 'STRING_REF', 'TILESET', 'TILE_TYPE', 'VARIATIONS']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<TilesData {pk_column}={pk_value!r}>>"
        else:
            return f"<TilesData>"
