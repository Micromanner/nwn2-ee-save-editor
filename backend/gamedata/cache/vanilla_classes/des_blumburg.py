...
class Des_blumburgData:
    """
    Auto-generated data class for des_blumburg.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Points')
    _column_mapping = {'Label': 'Label', 'Points': 'Points'}
    _table_name = 'des_blumburg'
    _safe_columns = ['Label', 'Points']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_blumburgData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_blumburgData>"
