...
class CursorsData:
    """
    Auto-generated data class for cursors.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_ResRef', '_CursorID')
    _column_mapping = {'Label': 'Label', 'ResRef': 'ResRef', 'CursorID': 'CursorID'}
    _table_name = 'cursors'
    _safe_columns = ['Label', 'ResRef', 'CursorID']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<CursorsData {pk_column}={pk_value!r}>>"
        else:
            return f"<CursorsData>"
