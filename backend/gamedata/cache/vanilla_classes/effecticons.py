...
class EffecticonsData:
    """
    Auto-generated data class for effecticons.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Icon', '_StrRef')
    _column_mapping = {'Label': 'Label', 'Icon': 'Icon', 'StrRef': 'StrRef'}
    _table_name = 'effecticons'
    _safe_columns = ['Label', 'Icon', 'StrRef']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<EffecticonsData {pk_column}={pk_value!r}>>"
        else:
            return f"<EffecticonsData>"
