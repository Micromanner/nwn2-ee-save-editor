...
class Nwn2_emotesData:
    """
    Auto-generated data class for nwn2_emotes.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Name', '_Icon', '_Animation', '_Duration', '_VoiceChat', '_Trigger', '_Command', '_KeyCode')
    _column_mapping = {'Label': 'Label', 'Name': 'Name', 'Icon': 'Icon', 'Animation': 'Animation', 'Duration': 'Duration', 'VoiceChat': 'VoiceChat', 'Trigger': 'Trigger', 'Command': 'Command', 'KeyCode': 'KeyCode'}
    _table_name = 'nwn2_emotes'
    _safe_columns = ['Label', 'Name', 'Icon', 'Animation', 'Duration', 'VoiceChat', 'Trigger', 'Command', 'KeyCode']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Nwn2_emotesData {pk_column}={pk_value!r}>>"
        else:
            return f"<Nwn2_emotesData>"
