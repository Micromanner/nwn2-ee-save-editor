...
class TimeData:
    """
    Auto-generated data class for time.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Image', '_Name', '_comments')
    _column_mapping = {'Label': 'Label', 'Image': 'Image', 'Name': 'Name', 'comments': 'comments'}
    _table_name = 'time'
    _safe_columns = ['Label', 'Image', 'Name', 'comments']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<TimeData {pk_column}={pk_value!r}>>"
        else:
            return f"<TimeData>"
