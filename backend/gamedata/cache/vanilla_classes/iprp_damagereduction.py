...
class Iprp_damagereductionData:
    """
    Auto-generated data class for iprp_damagereduction.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Cost')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Cost': 'Cost'}
    _table_name = 'iprp_damagereduction'
    _safe_columns = ['Name', 'Label', 'Cost']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_damagereductionData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_damagereductionData>"
