...
class Iprp_visualfxData:
    """
    Auto-generated data class for iprp_visualfx.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_col_empty_2', '_Label_Name', '_Effect')
    _column_mapping = {'': 'col_empty_2', 'Label                              Name': 'Label_Name', 'Effect': 'Effect'}
    _table_name = 'iprp_visualfx'
    _safe_columns = ['col_empty_2', 'Label_Name', 'Effect']
    ...
    def __repr__(self):
        ...
        pk_column = 'col_empty_2'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<Iprp_visualfxData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_visualfxData>"
