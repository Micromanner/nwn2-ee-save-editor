...
class EffectanimData:
    """
    Auto-generated data class for effectanim.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_ID', '_RADIUS')
    _column_mapping = {'ID': 'ID', 'RADIUS': 'RADIUS'}
    _table_name = 'effectanim'
    _safe_columns = ['ID', 'RADIUS']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<EffectanimData {pk_column}={pk_value!r}>>"
        else:
            return f"<EffectanimData>"
