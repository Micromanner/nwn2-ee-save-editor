...
class BodybagData:
    """
    Auto-generated data class for bodybag.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_Name', '_Appearance')
    _column_mapping = {'LABEL': 'LABEL', 'Name': 'Name', 'Appearance': 'Appearance'}
    _table_name = 'bodybag'
    _safe_columns = ['LABEL', 'Name', 'Appearance']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<BodybagData {pk_column}={pk_value!r}>>"
        else:
            return f"<BodybagData>"
