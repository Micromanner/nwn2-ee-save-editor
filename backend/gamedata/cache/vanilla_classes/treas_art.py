...
class Treas_artData:
    """
    Auto-generated data class for treas_art.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LTE', '_col_empty_2', '_DICE', '_NUM', '_TEMPLATE')
    _column_mapping = {'LTE': 'LTE', '': 'col_empty_2', 'DICE': 'DICE', 'NUM': 'NUM', 'TEMPLATE': 'TEMPLATE'}
    _table_name = 'treas_art'
    _safe_columns = ['LTE', 'col_empty_2', 'DICE', 'NUM', 'TEMPLATE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Treas_artData {pk_column}={pk_value!r}>>"
        else:
            return f"<Treas_artData>"
