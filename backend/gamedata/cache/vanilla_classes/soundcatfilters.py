...
class SoundcatfiltersData:
    """
    Auto-generated data class for soundcatfilters.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Filter')
    _column_mapping = {'Label': 'Label', 'Filter': 'Filter'}
    _table_name = 'soundcatfilters'
    _safe_columns = ['Label', 'Filter']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SoundcatfiltersData {pk_column}={pk_value!r}>>"
        else:
            return f"<SoundcatfiltersData>"
