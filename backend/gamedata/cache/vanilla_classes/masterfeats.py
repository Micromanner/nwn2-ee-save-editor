...
class MasterfeatsData:
    """
    Auto-generated data class for masterfeats.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_STRREF', '_DESCRIPTION', '_ICON')
    _column_mapping = {'LABEL': 'LABEL', 'STRREF': 'STRREF', 'DESCRIPTION': 'DESCRIPTION', 'ICON': 'ICON'}
    _table_name = 'masterfeats'
    _safe_columns = ['LABEL', 'STRREF', 'DESCRIPTION', 'ICON']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<MasterfeatsData {pk_column}={pk_value!r}>>"
        else:
            return f"<MasterfeatsData>"
