...
class Iprp_arcspellData:
    """
    Auto-generated data class for iprp_arcspell.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Value', '_Cost')
    _column_mapping = {'Name': 'Name', 'Value': 'Value', 'Cost': 'Cost'}
    _table_name = 'iprp_arcspell'
    _safe_columns = ['Name', 'Value', 'Cost']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_arcspellData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_arcspellData>"
