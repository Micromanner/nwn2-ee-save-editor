...
class Cls_skill_spiritshamanData:
    """
    Auto-generated data class for cls_skill_spiritshaman.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_SkillLabel', '_SkillIndex', '_ClassSkill')
    _column_mapping = {'SkillLabel': 'SkillLabel', 'SkillIndex': 'SkillIndex', 'ClassSkill': 'ClassSkill'}
    _table_name = 'cls_skill_spiritshaman'
    _safe_columns = ['SkillLabel', 'SkillIndex', 'ClassSkill']
    ...
    def __repr__(self):
        ...
        pk_column = 'SkillLabel'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<Cls_skill_spiritshamanData {pk_column}={pk_value!r}>>"
        else:
            return f"<Cls_skill_spiritshamanData>"
