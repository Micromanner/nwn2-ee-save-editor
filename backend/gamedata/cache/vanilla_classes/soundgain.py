...
class SoundgainData:
    """
    Auto-generated data class for soundgain.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Volume', '_MinDistance', '_MaxDistance')
    _column_mapping = {'Name': 'Name', 'Volume': 'Volume', 'MinDistance': 'MinDistance', 'MaxDistance': 'MaxDistance'}
    _table_name = 'soundgain'
    _safe_columns = ['Name', 'Volume', 'MinDistance', 'MaxDistance']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SoundgainData {pk_column}={pk_value!r}>>"
        else:
            return f"<SoundgainData>"
