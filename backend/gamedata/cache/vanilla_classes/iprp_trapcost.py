...
class Iprp_trapcostData:
    """
    Auto-generated data class for iprp_trapcost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Cost', '_Minor', '_Average', '_Strong', '_Deadly', '_Epic')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Cost': 'Cost', 'Minor': 'Minor', 'Average': 'Average', 'Strong': 'Strong', 'Deadly': 'Deadly', 'Epic': 'Epic'}
    _table_name = 'iprp_trapcost'
    _safe_columns = ['Name', 'Label', 'Cost', 'Minor', 'Average', 'Strong', 'Deadly', 'Epic']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_trapcostData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_trapcostData>"
