...
class Iprp_onhitdurData:
    """
    Auto-generated data class for iprp_onhitdur.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Cost', '_EffectChance', '_DurationRounds')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Cost': 'Cost', 'EffectChance': 'EffectChance', 'DurationRounds': 'DurationRounds'}
    _table_name = 'iprp_onhitdur'
    _safe_columns = ['Name', 'Label', 'Cost', 'EffectChance', 'DurationRounds']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_onhitdurData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_onhitdurData>"
