...
class Texture_replaceData:
    """
    Auto-generated data class for texture_replace.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_From_', '_To')
    _column_mapping = {'From': 'From_', 'To': 'To'}
    _table_name = 'texture_replace'
    _safe_columns = ['From_', 'To']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Texture_replaceData {pk_column}={pk_value!r}>>"
        else:
            return f"<Texture_replaceData>"
