...
class Iprp_weightincData:
    """
    Auto-generated data class for iprp_weightinc.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Value')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Value': 'Value'}
    _table_name = 'iprp_weightinc'
    _safe_columns = ['Name', 'Label', 'Value']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_weightincData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_weightincData>"
