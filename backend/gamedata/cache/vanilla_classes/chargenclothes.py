...
class ChargenclothesData:
    """
    Auto-generated data class for chargenclothes.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_ItemResRef')
    _column_mapping = {'ItemResRef': 'ItemResRef'}
    _table_name = 'chargenclothes'
    _safe_columns = ['ItemResRef']
    ...
    def __repr__(self):
        ...
        pk_column = 'ItemResRef'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<ChargenclothesData {pk_column}={pk_value!r}>>"
        else:
            return f"<ChargenclothesData>"
