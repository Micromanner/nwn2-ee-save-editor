...
class SkyboxesData:
    """
    Auto-generated data class for skyboxes.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_STRING_REF', '_CYCLICAL', '_DAWN', '_DAY', '_DUSK', '_NIGHT')
    _column_mapping = {'LABEL': 'LABEL', 'STRING_REF': 'STRING_REF', 'CYCLICAL': 'CYCLICAL', 'DAWN': 'DAWN', 'DAY': 'DAY', 'DUSK': 'DUSK', 'NIGHT': 'NIGHT'}
    _table_name = 'skyboxes'
    _safe_columns = ['LABEL', 'STRING_REF', 'CYCLICAL', 'DAWN', 'DAY', 'DUSK', 'NIGHT']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SkyboxesData {pk_column}={pk_value!r}>>"
        else:
            return f"<SkyboxesData>"
