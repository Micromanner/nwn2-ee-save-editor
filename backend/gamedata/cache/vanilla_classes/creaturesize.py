...
class CreaturesizeData:
    """
    Auto-generated data class for creaturesize.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_ACATTACKMOD', '_STRREF')
    _column_mapping = {'LABEL': 'LABEL', 'ACATTACKMOD': 'ACATTACKMOD', 'STRREF': 'STRREF'}
    _table_name = 'creaturesize'
    _safe_columns = ['LABEL', 'ACATTACKMOD', 'STRREF']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<CreaturesizeData {pk_column}={pk_value!r}>>"
        else:
            return f"<CreaturesizeData>"
