...
class DiffsettingsData:
    """
    Auto-generated data class for diffsettings.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_DMEasy', '_Easy', '_Normal', '_Hardcore', '_DMPlayers')
    _column_mapping = {'Name': 'Name', 'DMEasy': 'DMEasy', 'Easy': 'Easy', 'Normal': 'Normal', 'Hardcore': 'Hardcore', 'DMPlayers': 'DMPlayers'}
    _table_name = 'diffsettings'
    _safe_columns = ['Name', 'DMEasy', 'Easy', 'Normal', 'Hardcore', 'DMPlayers']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<DiffsettingsData {pk_column}={pk_value!r}>>"
        else:
            return f"<DiffsettingsData>"
