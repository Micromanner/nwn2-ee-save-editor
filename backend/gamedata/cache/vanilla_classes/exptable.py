...
class ExptableData:
    """
    Auto-generated data class for exptable.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Level', '_XP')
    _column_mapping = {'Level': 'Level', 'XP': 'XP'}
    _table_name = 'exptable'
    _safe_columns = ['Level', 'XP']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ExptableData {pk_column}={pk_value!r}>>"
        else:
            return f"<ExptableData>"
