...
class LightcolorData:
    """
    Auto-generated data class for lightcolor.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_RED', '_GREEN', '_BLUE', '_LABEL', '_TOOLSETRED', '_TOOLSETGREEN', '_TOOLSETBLUE')
    _column_mapping = {'RED': 'RED', 'GREEN': 'GREEN', 'BLUE': 'BLUE', 'LABEL': 'LABEL', 'TOOLSETRED': 'TOOLSETRED', 'TOOLSETGREEN': 'TOOLSETGREEN', 'TOOLSETBLUE': 'TOOLSETBLUE'}
    _table_name = 'lightcolor'
    _safe_columns = ['RED', 'GREEN', 'BLUE', 'LABEL', 'TOOLSETRED', 'TOOLSETGREEN', 'TOOLSETBLUE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<LightcolorData {pk_column}={pk_value!r}>>"
        else:
            return f"<LightcolorData>"
