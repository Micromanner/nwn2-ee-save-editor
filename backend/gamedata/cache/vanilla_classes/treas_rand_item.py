...
class Treas_rand_itemData:
    """
    Auto-generated data class for treas_rand_item.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LEVEL', '_LTE', '_col_empty_2', '_DIE', '_NUM', '_ITEM_LEVEL', '_ITEM_TABLE')
    _column_mapping = {'LEVEL': 'LEVEL', 'LTE': 'LTE', '': 'col_empty_2', 'DIE': 'DIE', 'NUM': 'NUM', 'ITEM_LEVEL': 'ITEM_LEVEL', 'ITEM_TABLE': 'ITEM_TABLE'}
    _table_name = 'treas_rand_item'
    _safe_columns = ['LEVEL', 'LTE', 'col_empty_2', 'DIE', 'NUM', 'ITEM_LEVEL', 'ITEM_TABLE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Treas_rand_itemData {pk_column}={pk_value!r}>>"
        else:
            return f"<Treas_rand_itemData>"
