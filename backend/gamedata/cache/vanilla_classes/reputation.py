...
class ReputationData:
    """
    Auto-generated data class for reputation.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Idolized', '_Honored', '_Liked', '_Known', '_Unwelcome', '_Disliked', '_Loathed', '_Vilified')
    _column_mapping = {'Label': 'Label', 'Idolized': 'Idolized', 'Honored': 'Honored', 'Liked': 'Liked', 'Known': 'Known', 'Unwelcome': 'Unwelcome', 'Disliked': 'Disliked', 'Loathed': 'Loathed', 'Vilified': 'Vilified'}
    _table_name = 'reputation'
    _safe_columns = ['Label', 'Idolized', 'Honored', 'Liked', 'Known', 'Unwelcome', 'Disliked', 'Loathed', 'Vilified']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ReputationData {pk_column}={pk_value!r}>>"
        else:
            return f"<ReputationData>"
