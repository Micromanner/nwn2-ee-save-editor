...
class SkillvsitemcostData:
    """
    Auto-generated data class for skillvsitemcost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_DeviceCostMax', '_SkillReq_Class', '_SkillReq_Race', '_SkillReq_Align', '_DeviceCostMaxLore')
    _column_mapping = {'DeviceCostMax': 'DeviceCostMax', 'SkillReq_Class': 'SkillReq_Class', 'SkillReq_Race': 'SkillReq_Race', 'SkillReq_Align': 'SkillReq_Align', 'DeviceCostMaxLore': 'DeviceCostMaxLore'}
    _table_name = 'skillvsitemcost'
    _safe_columns = ['DeviceCostMax', 'SkillReq_Class', 'SkillReq_Race', 'SkillReq_Align', 'DeviceCostMaxLore']
    ...
    def __repr__(self):
        ...
        pk_column = 'DeviceCostMax'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<SkillvsitemcostData {pk_column}={pk_value!r}>>"
        else:
            return f"<SkillvsitemcostData>"
