...
class DamagehitvisualData:
    """
    Auto-generated data class for damagehitvisual.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_VisualEffectID', '_RangedEffectID')
    _column_mapping = {'Label': 'Label', 'VisualEffectID': 'VisualEffectID', 'RangedEffectID': 'RangedEffectID'}
    _table_name = 'damagehitvisual'
    _safe_columns = ['Label', 'VisualEffectID', 'RangedEffectID']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<DamagehitvisualData {pk_column}={pk_value!r}>>"
        else:
            return f"<DamagehitvisualData>"
