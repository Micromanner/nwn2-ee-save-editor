...
class ReputeData:
    """
    Auto-generated data class for repute.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_HOSTILE', '_COMMONER', '_MERCHANT', '_DEFENDER')
    _column_mapping = {'LABEL': 'LABEL', 'HOSTILE': 'HOSTILE', 'COMMONER': 'COMMONER', 'MERCHANT': 'MERCHANT', 'DEFENDER': 'DEFENDER'}
    _table_name = 'repute'
    _safe_columns = ['LABEL', 'HOSTILE', 'COMMONER', 'MERCHANT', 'DEFENDER']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ReputeData {pk_column}={pk_value!r}>>"
        else:
            return f"<ReputeData>"
