...
class Des_crft_spellsData:
    """
    Auto-generated data class for des_crft_spells.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_IPRP_SpellIndex', '_NoPotion', '_NoWand', '_NoScroll', '_Level', '_CastOnItems')
    _column_mapping = {'Label': 'Label', 'IPRP_SpellIndex': 'IPRP_SpellIndex', 'NoPotion': 'NoPotion', 'NoWand': 'NoWand', 'NoScroll': 'NoScroll', 'Level': 'Level', 'CastOnItems': 'CastOnItems'}
    _table_name = 'des_crft_spells'
    _safe_columns = ['Label', 'IPRP_SpellIndex', 'NoPotion', 'NoWand', 'NoScroll', 'Level', 'CastOnItems']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_crft_spellsData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_crft_spellsData>"
