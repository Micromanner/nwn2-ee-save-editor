...
class GrassData:
    """
    Auto-generated data class for grass.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_STR_REF', '_col_empty', '_Texture')
    _column_mapping = {'STR_REF': 'STR_REF', '': 'col_empty', 'Texture': 'Texture'}
    _table_name = 'grass'
    _safe_columns = ['STR_REF', 'col_empty', 'Texture']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<GrassData {pk_column}={pk_value!r}>>"
        else:
            return f"<GrassData>"
