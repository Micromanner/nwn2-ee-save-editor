...
class VideoqualityData:
    """
    Auto-generated data class for videoquality.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_col_empty_10', '_INILABEL', '_FAST', '_LOW', '_GOOD', '_BEST')
    _column_mapping = {'': 'col_empty_10', 'INILABEL': 'INILABEL', 'FAST': 'FAST', 'LOW': 'LOW', 'GOOD': 'GOOD', 'BEST': 'BEST'}
    _table_name = 'videoquality'
    _safe_columns = ['col_empty_10', 'INILABEL', 'FAST', 'LOW', 'GOOD', 'BEST']
    ...
    def __repr__(self):
        ...
        pk_column = 'col_empty_10'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<VideoqualityData {pk_column}={pk_value!r}>>"
        else:
            return f"<VideoqualityData>"
