...
class CreaturespeedData:
    """
    Auto-generated data class for creaturespeed.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Name', '_col_2DAName', '_WALKRATE', '_RUNRATE')
    _column_mapping = {'Label': 'Label', 'Name': 'Name', '2DAName': 'col_2DAName', 'WALKRATE': 'WALKRATE', 'RUNRATE': 'RUNRATE'}
    _table_name = 'creaturespeed'
    _safe_columns = ['Label', 'Name', 'col_2DAName', 'WALKRATE', 'RUNRATE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<CreaturespeedData {pk_column}={pk_value!r}>>"
        else:
            return f"<CreaturespeedData>"
