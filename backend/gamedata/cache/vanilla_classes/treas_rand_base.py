...
class Treas_rand_baseData:
    """
    Auto-generated data class for treas_rand_base.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_GP_TABLE', '_col_empty_2', '_GEM_TABLE', '_ITEM_TABLE')
    _column_mapping = {'GP_TABLE': 'GP_TABLE', '': 'col_empty_2', 'GEM_TABLE': 'GEM_TABLE', 'ITEM_TABLE': 'ITEM_TABLE'}
    _table_name = 'treas_rand_base'
    _safe_columns = ['GP_TABLE', 'col_empty_2', 'GEM_TABLE', 'ITEM_TABLE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Treas_rand_baseData {pk_column}={pk_value!r}>>"
        else:
            return f"<Treas_rand_baseData>"
