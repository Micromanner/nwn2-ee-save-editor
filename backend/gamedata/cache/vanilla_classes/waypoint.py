...
class WaypointData:
    """
    Auto-generated data class for waypoint.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_NWN2_ModelName', '_RESREF', '_STRREF')
    _column_mapping = {'LABEL': 'LABEL', 'NWN2_ModelName': 'NWN2_ModelName', 'RESREF': 'RESREF', 'STRREF': 'STRREF'}
    _table_name = 'waypoint'
    _safe_columns = ['LABEL', 'NWN2_ModelName', 'RESREF', 'STRREF']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<WaypointData {pk_column}={pk_value!r}>>"
        else:
            return f"<WaypointData>"
