...
class Des_crft_appearData:
    """
    Auto-generated data class for des_crft_appear.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Next', '_Prev', '_BaseAC')
    _column_mapping = {'Next': 'Next', 'Prev': 'Prev', 'BaseAC': 'BaseAC'}
    _table_name = 'des_crft_appear'
    _safe_columns = ['Next', 'Prev', 'BaseAC']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_crft_appearData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_crft_appearData>"
