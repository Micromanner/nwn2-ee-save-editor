...
class EncumbranceData:
    """
    Auto-generated data class for encumbrance.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Normal', '_Heavy')
    _column_mapping = {'Normal': 'Normal', 'Heavy': 'Heavy'}
    _table_name = 'encumbrance'
    _safe_columns = ['Normal', 'Heavy']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<EncumbranceData {pk_column}={pk_value!r}>>"
        else:
            return f"<EncumbranceData>"
