...
class SoundeaxData:
    """
    Auto-generated data class for soundeax.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Description')
    _column_mapping = {'Label': 'Label', 'Description': 'Description'}
    _table_name = 'soundeax'
    _safe_columns = ['Label', 'Description']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SoundeaxData {pk_column}={pk_value!r}>>"
        else:
            return f"<SoundeaxData>"
