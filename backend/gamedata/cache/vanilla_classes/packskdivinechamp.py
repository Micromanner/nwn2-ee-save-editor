...
class PackskdivinechampData:
    """
    Auto-generated data class for packskdivinechamp.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_SkillIndex', '_Label')
    _column_mapping = {'SkillIndex': 'SkillIndex', 'Label': 'Label'}
    _table_name = 'packskdivinechamp'
    _safe_columns = ['SkillIndex', 'Label']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<PackskdivinechampData {pk_column}={pk_value!r}>>"
        else:
            return f"<PackskdivinechampData>"
