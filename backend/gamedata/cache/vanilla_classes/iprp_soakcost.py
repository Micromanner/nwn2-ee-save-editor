...
class Iprp_soakcostData:
    """
    Auto-generated data class for iprp_soakcost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name_Label_Amount_Cost', '_col_empty_4')
    _column_mapping = {'Name   Label    Amount   Cost': 'Name_Label_Amount_Cost', '': 'col_empty_4'}
    _table_name = 'iprp_soakcost'
    _safe_columns = ['Name_Label_Amount_Cost', 'col_empty_4']
    ...
    def __repr__(self):
        ...
        pk_column = 'Name_Label_Amount_Cost'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<Iprp_soakcostData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_soakcostData>"
