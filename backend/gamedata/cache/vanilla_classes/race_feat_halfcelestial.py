...
class Race_feat_halfcelestialData:
    """
    Auto-generated data class for race_feat_halfcelestial.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_FeatLabel', '_FeatIndex')
    _column_mapping = {'FeatLabel': 'FeatLabel', 'FeatIndex': 'FeatIndex'}
    _table_name = 'race_feat_halfcelestial'
    _safe_columns = ['FeatLabel', 'FeatIndex']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Race_feat_halfcelestialData {pk_column}={pk_value!r}>>"
        else:
            return f"<Race_feat_halfcelestialData>"
