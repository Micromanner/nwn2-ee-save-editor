...
class PortraitsData:
    """
    Auto-generated data class for portraits.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_BaseResRef', '_Sex', '_Race', '_InanimateType', '_Plot', '_LowGore')
    _column_mapping = {'BaseResRef': 'BaseResRef', 'Sex': 'Sex', 'Race': 'Race', 'InanimateType': 'InanimateType', 'Plot': 'Plot', 'LowGore': 'LowGore'}
    _table_name = 'portraits'
    _safe_columns = ['BaseResRef', 'Sex', 'Race', 'InanimateType', 'Plot', 'LowGore']
    ...
    def __repr__(self):
        ...
        pk_column = 'BaseResRef'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<PortraitsData {pk_column}={pk_value!r}>>"
        else:
            return f"<PortraitsData>"
