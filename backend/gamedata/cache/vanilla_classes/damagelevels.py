...
class DamagelevelsData:
    """
    Auto-generated data class for damagelevels.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_STRING_REF', '_COLOR', '_BOLD', '_DMG_LEVEL')
    _column_mapping = {'LABEL': 'LABEL', 'STRING_REF': 'STRING_REF', 'COLOR': 'COLOR', 'BOLD': 'BOLD', 'DMG_LEVEL': 'DMG_LEVEL'}
    _table_name = 'damagelevels'
    _safe_columns = ['LABEL', 'STRING_REF', 'COLOR', 'BOLD', 'DMG_LEVEL']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<DamagelevelsData {pk_column}={pk_value!r}>>"
        else:
            return f"<DamagelevelsData>"
