...
class SoundsetData:
    """
    Auto-generated data class for soundset.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_RESREF', '_STRREF', '_GENDER', '_TYPE_')
    _column_mapping = {'LABEL': 'LABEL', 'RESREF': 'RESREF', 'STRREF': 'STRREF', 'GENDER': 'GENDER', 'TYPE': 'TYPE_'}
    _table_name = 'soundset'
    _safe_columns = ['LABEL', 'RESREF', 'STRREF', 'GENDER', 'TYPE_']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SoundsetData {pk_column}={pk_value!r}>>"
        else:
            return f"<SoundsetData>"
