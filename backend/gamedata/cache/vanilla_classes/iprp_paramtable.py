...
class Iprp_paramtableData:
    """
    Auto-generated data class for iprp_paramtable.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Lable', '_TableResRef')
    _column_mapping = {'Name': 'Name', 'Lable': 'Lable', 'TableResRef': 'TableResRef'}
    _table_name = 'iprp_paramtable'
    _safe_columns = ['Name', 'Lable', 'TableResRef']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_paramtableData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_paramtableData>"
