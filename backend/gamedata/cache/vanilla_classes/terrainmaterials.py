...
class TerrainmaterialsData:
    """
    Auto-generated data class for terrainmaterials.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Terrain', '_R', '_G', '_B', '_Material', '_STR_REF')
    _column_mapping = {'Terrain': 'Terrain', 'R': 'R', 'G': 'G', 'B': 'B', 'Material': 'Material', 'STR_REF': 'STR_REF'}
    _table_name = 'terrainmaterials'
    _safe_columns = ['Terrain', 'R', 'G', 'B', 'Material', 'STR_REF']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<TerrainmaterialsData {pk_column}={pk_value!r}>>"
        else:
            return f"<TerrainmaterialsData>"
