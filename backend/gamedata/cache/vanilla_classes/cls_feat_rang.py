...
class Cls_feat_rangData:
    """
    Auto-generated data class for cls_feat_rang.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_FeatLabel', '_FeatIndex', '_List', '_GrantedOnLevel', '_OnMenu', '_GrantedPrereq')
    _column_mapping = {'FeatLabel': 'FeatLabel', 'FeatIndex': 'FeatIndex', 'List': 'List', 'GrantedOnLevel': 'GrantedOnLevel', 'OnMenu': 'OnMenu', 'GrantedPrereq': 'GrantedPrereq'}
    _table_name = 'cls_feat_rang'
    _safe_columns = ['FeatLabel', 'FeatIndex', 'List', 'GrantedOnLevel', 'OnMenu', 'GrantedPrereq']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Cls_feat_rangData {pk_column}={pk_value!r}>>"
        else:
            return f"<Cls_feat_rangData>"
