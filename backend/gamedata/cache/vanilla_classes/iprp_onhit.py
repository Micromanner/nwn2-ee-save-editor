...
class Iprp_onhitData:
    """
    Auto-generated data class for iprp_onhit.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Cost', '_Param1ResRef')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Cost': 'Cost', 'Param1ResRef': 'Param1ResRef'}
    _table_name = 'iprp_onhit'
    _safe_columns = ['Name', 'Label', 'Cost', 'Param1ResRef']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_onhitData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_onhitData>"
