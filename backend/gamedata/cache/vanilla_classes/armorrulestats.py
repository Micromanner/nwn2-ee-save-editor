...
class ArmorrulestatsData:
    """
    Auto-generated data class for armorrulestats.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_ACBONUS', '_MAXDEXBONUS', '_ACCHECK', '_ARCANEFAILUREpct', '_WEIGHT', '_COST', '_DESCRIPTIONS', '_Name', '_BASEITEMSTATREF', '_RANK')
    _column_mapping = {'Label': 'Label', 'ACBONUS': 'ACBONUS', 'MAXDEXBONUS': 'MAXDEXBONUS', 'ACCHECK': 'ACCHECK', 'ARCANEFAILURE%': 'ARCANEFAILUREpct', 'WEIGHT': 'WEIGHT', 'COST': 'COST', 'DESCRIPTIONS': 'DESCRIPTIONS', 'Name': 'Name', 'BASEITEMSTATREF': 'BASEITEMSTATREF', 'RANK': 'RANK'}
    _table_name = 'armorrulestats'
    _safe_columns = ['Label', 'ACBONUS', 'MAXDEXBONUS', 'ACCHECK', 'ARCANEFAILUREpct', 'WEIGHT', 'COST', 'DESCRIPTIONS', 'Name', 'BASEITEMSTATREF', 'RANK']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ArmorrulestatsData {pk_column}={pk_value!r}>>"
        else:
            return f"<ArmorrulestatsData>"
