...
class Treas_rand_gemData:
    """
    Auto-generated data class for treas_rand_gem.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LEVEL', '_LTE', '_col_empty', '_GEM_TABLE', '_DIE', '_NUM')
    _column_mapping = {'LEVEL': 'LEVEL', 'LTE': 'LTE', '': 'col_empty', 'GEM_TABLE': 'GEM_TABLE', 'DIE': 'DIE', 'NUM': 'NUM'}
    _table_name = 'treas_rand_gem'
    _safe_columns = ['LEVEL', 'LTE', 'col_empty', 'GEM_TABLE', 'DIE', 'NUM']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Treas_rand_gemData {pk_column}={pk_value!r}>>"
        else:
            return f"<Treas_rand_gemData>"
