...
class SpellschoolsData:
    """
    Auto-generated data class for spellschools.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Letter', '_StringRef', '_Icon', '_Opposition', '_Description', '_SecondOpposition')
    _column_mapping = {'Label': 'Label', 'Letter': 'Letter', 'StringRef': 'StringRef', 'Icon': 'Icon', 'Opposition': 'Opposition', 'Description': 'Description', 'SecondOpposition': 'SecondOpposition'}
    _table_name = 'spellschools'
    _safe_columns = ['Label', 'Letter', 'StringRef', 'Icon', 'Opposition', 'Description', 'SecondOpposition']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<SpellschoolsData {pk_column}={pk_value!r}>>"
        else:
            return f"<SpellschoolsData>"
