...
class ExciteddurationData:
    """
    Auto-generated data class for excitedduration.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_DURATION')
    _column_mapping = {'LABEL': 'LABEL', 'DURATION': 'DURATION'}
    _table_name = 'excitedduration'
    _safe_columns = ['LABEL', 'DURATION']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ExciteddurationData {pk_column}={pk_value!r}>>"
        else:
            return f"<ExciteddurationData>"
