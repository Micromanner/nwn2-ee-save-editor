...
class Cls_spon_clerData:
    """
    Auto-generated data class for cls_spon_cler.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Level', '_SpellId', '_SpellName', '_OverlayIcon', '_AltSpellId', '_AltSpellName', '_AltOverlayIcon')
    _column_mapping = {'Level': 'Level', 'SpellId': 'SpellId', 'SpellName': 'SpellName', 'OverlayIcon': 'OverlayIcon', 'AltSpellId': 'AltSpellId', 'AltSpellName': 'AltSpellName', 'AltOverlayIcon': 'AltOverlayIcon'}
    _table_name = 'cls_spon_cler'
    _safe_columns = ['Level', 'SpellId', 'SpellName', 'OverlayIcon', 'AltSpellId', 'AltSpellName', 'AltOverlayIcon']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Cls_spon_clerData {pk_column}={pk_value!r}>>"
        else:
            return f"<Cls_spon_clerData>"
