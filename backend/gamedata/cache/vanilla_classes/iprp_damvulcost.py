...
class Iprp_damvulcostData:
    """
    Auto-generated data class for iprp_damvulcost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Value', '_Cost')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Value': 'Value', 'Cost': 'Cost'}
    _table_name = 'iprp_damvulcost'
    _safe_columns = ['Name', 'Label', 'Value', 'Cost']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_damvulcostData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_damvulcostData>"
