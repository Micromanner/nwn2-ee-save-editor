...
class Iprp_spellsData:
    """
    Auto-generated data class for iprp_spells.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Name', '_CasterLvl', '_InnateLvl', '_Cost', '_SpellIndex', '_PotionUse', '_WandUse', '_GeneralUse', '_Icon')
    _column_mapping = {'Label': 'Label', 'Name': 'Name', 'CasterLvl': 'CasterLvl', 'InnateLvl': 'InnateLvl', 'Cost': 'Cost', 'SpellIndex': 'SpellIndex', 'PotionUse': 'PotionUse', 'WandUse': 'WandUse', 'GeneralUse': 'GeneralUse', 'Icon': 'Icon'}
    _table_name = 'iprp_spells'
    _safe_columns = ['Label', 'Name', 'CasterLvl', 'InnateLvl', 'Cost', 'SpellIndex', 'PotionUse', 'WandUse', 'GeneralUse', 'Icon']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_spellsData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_spellsData>"
