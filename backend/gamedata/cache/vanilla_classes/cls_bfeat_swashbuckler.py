...
class Cls_bfeat_swashbucklerData:
    """
    Auto-generated data class for cls_bfeat_swashbuckler.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Bonus', '_Normal')
    _column_mapping = {'Bonus': 'Bonus', 'Normal': 'Normal'}
    _table_name = 'cls_bfeat_swashbuckler'
    _safe_columns = ['Bonus', 'Normal']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Cls_bfeat_swashbucklerData {pk_column}={pk_value!r}>>"
        else:
            return f"<Cls_bfeat_swashbucklerData>"
