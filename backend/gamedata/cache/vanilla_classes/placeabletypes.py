...
class PlaceabletypesData:
    """
    Auto-generated data class for placeabletypes.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_StrRef')
    _column_mapping = {'Label': 'Label', 'StrRef': 'StrRef'}
    _table_name = 'placeabletypes'
    _safe_columns = ['Label', 'StrRef']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<PlaceabletypesData {pk_column}={pk_value!r}>>"
        else:
            return f"<PlaceabletypesData>"
