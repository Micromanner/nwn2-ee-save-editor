...
class TilesetsData:
    """
    Auto-generated data class for tilesets.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_STRING_REF', '_TILESET')
    _column_mapping = {'LABEL': 'LABEL', 'STRING_REF': 'STRING_REF', 'TILESET': 'TILESET'}
    _table_name = 'tilesets'
    _safe_columns = ['LABEL', 'STRING_REF', 'TILESET']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<TilesetsData {pk_column}={pk_value!r}>>"
        else:
            return f"<TilesetsData>"
