...
class Vfx_fire_forgetData:
    """
    Auto-generated data class for vfx_fire_forget.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_MODEL')
    _column_mapping = {'LABEL': 'LABEL', 'MODEL': 'MODEL'}
    _table_name = 'vfx_fire_forget'
    _safe_columns = ['LABEL', 'MODEL']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Vfx_fire_forgetData {pk_column}={pk_value!r}>>"
        else:
            return f"<Vfx_fire_forgetData>"
