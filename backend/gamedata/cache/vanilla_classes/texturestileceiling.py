...
class TexturestileceilingData:
    """
    Auto-generated data class for texturestileceiling.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_Texture', '_TileSetID', '_Default')
    _column_mapping = {'LABEL': 'LABEL', 'Texture': 'Texture', 'TileSetID': 'TileSetID', 'Default': 'Default'}
    _table_name = 'texturestileceiling'
    _safe_columns = ['LABEL', 'Texture', 'TileSetID', 'Default']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<TexturestileceilingData {pk_column}={pk_value!r}>>"
        else:
            return f"<TexturestileceilingData>"
