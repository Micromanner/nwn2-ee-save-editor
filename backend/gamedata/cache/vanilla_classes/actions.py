...
class ActionsData:
    """
    Auto-generated data class for actions.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_STRING_REF', '_ICONRESREF', '_TIMER')
    _column_mapping = {'LABEL': 'LABEL', 'STRING_REF': 'STRING_REF', 'ICONRESREF': 'ICONRESREF', 'TIMER': 'TIMER'}
    _table_name = 'actions'
    _safe_columns = ['LABEL', 'STRING_REF', 'ICONRESREF', 'TIMER']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ActionsData {pk_column}={pk_value!r}>>"
        else:
            return f"<ActionsData>"
