...
class Des_mechupgradesData:
    """
    Auto-generated data class for des_mechupgrades.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Name', '_Description', '_InvSlot', '_col_empty', '_permanent')
    _column_mapping = {'Label': 'Label', 'Name': 'Name', 'Description': 'Description', 'InvSlot': 'InvSlot', '': 'col_empty', 'permanent': 'permanent'}
    _table_name = 'des_mechupgrades'
    _safe_columns = ['Label', 'Name', 'Description', 'InvSlot', 'col_empty', 'permanent']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_mechupgradesData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_mechupgradesData>"
