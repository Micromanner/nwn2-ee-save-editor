...
class Packskfight8Data:
    """
    Auto-generated data class for packskfight8.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_SkillIndex', '_Label', '_col_empty_2')
    _column_mapping = {'SkillIndex': 'SkillIndex', 'Label': 'Label', '': 'col_empty_2'}
    _table_name = 'packskfight8'
    _safe_columns = ['SkillIndex', 'Label', 'col_empty_2']
    ...
    def __repr__(self):
        ...
        pk_column = 'SkillIndex'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<Packskfight8Data {pk_column}={pk_value!r}>>"
        else:
            return f"<Packskfight8Data>"
