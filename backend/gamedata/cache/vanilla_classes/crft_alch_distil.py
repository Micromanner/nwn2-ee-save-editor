...
class Crft_alch_distilData:
    """
    Auto-generated data class for crft_alch_distil.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Name', '_Description', '_SourceItem', '_EssenceItem', '_DC')
    _column_mapping = {'Label': 'Label', 'Name': 'Name', 'Description': 'Description', 'SourceItem': 'SourceItem', 'EssenceItem': 'EssenceItem', 'DC': 'DC'}
    _table_name = 'crft_alch_distil'
    _safe_columns = ['Label', 'Name', 'Description', 'SourceItem', 'EssenceItem', 'DC']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Crft_alch_distilData {pk_column}={pk_value!r}>>"
        else:
            return f"<Crft_alch_distilData>"
