...
class Treas_rand_gpData:
    """
    Auto-generated data class for treas_rand_gp.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LEVEL', '_LTE', '_col_empty_3', '_DIE', '_NUM')
    _column_mapping = {'LEVEL': 'LEVEL', 'LTE': 'LTE', '': 'col_empty_3', 'DIE': 'DIE', 'NUM': 'NUM'}
    _table_name = 'treas_rand_gp'
    _safe_columns = ['LEVEL', 'LTE', 'col_empty_3', 'DIE', 'NUM']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Treas_rand_gpData {pk_column}={pk_value!r}>>"
        else:
            return f"<Treas_rand_gpData>"
