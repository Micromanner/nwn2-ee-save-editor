...
class Screen_container_uiData:
    """
    Auto-generated data class for screen_container_ui.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_SCREEN', '_SCREEN_EE')
    _column_mapping = {'SCREEN': 'SCREEN', 'SCREEN_EE': 'SCREEN_EE'}
    _table_name = 'screen_container_ui'
    _safe_columns = ['SCREEN', 'SCREEN_EE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Screen_container_uiData {pk_column}={pk_value!r}>>"
        else:
            return f"<Screen_container_uiData>"
