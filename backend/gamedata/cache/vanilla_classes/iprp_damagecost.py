...
class Iprp_damagecostData:
    """
    Auto-generated data class for iprp_damagecost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Cost', '_NumDice', '_Die', '_Rank', '_GameString', '_VFX', '_ExpectedRank')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Cost': 'Cost', 'NumDice': 'NumDice', 'Die': 'Die', 'Rank': 'Rank', 'GameString': 'GameString', 'VFX': 'VFX', 'ExpectedRank': 'ExpectedRank'}
    _table_name = 'iprp_damagecost'
    _safe_columns = ['Name', 'Label', 'Cost', 'NumDice', 'Die', 'Rank', 'GameString', 'VFX', 'ExpectedRank']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_damagecostData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_damagecostData>"
