...
class Itm_rand_armorData:
    """
    Auto-generated data class for itm_rand_armor.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LTE_MINOR', '_LTE_MEDIUM', '_LTE_MAJOR', '_ITEM', '_col_empty', '_BONUS', '_BASE_ITEM_TABLE')
    _column_mapping = {'LTE_MINOR': 'LTE_MINOR', 'LTE_MEDIUM': 'LTE_MEDIUM', 'LTE_MAJOR': 'LTE_MAJOR', 'ITEM': 'ITEM', '': 'col_empty', 'BONUS': 'BONUS', 'BASE_ITEM_TABLE': 'BASE_ITEM_TABLE'}
    _table_name = 'itm_rand_armor'
    _safe_columns = ['LTE_MINOR', 'LTE_MEDIUM', 'LTE_MAJOR', 'ITEM', 'col_empty', 'BONUS', 'BASE_ITEM_TABLE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Itm_rand_armorData {pk_column}={pk_value!r}>>"
        else:
            return f"<Itm_rand_armorData>"
