...
class FractionalcrData:
    """
    Auto-generated data class for fractionalcr.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_DisplayStrRef', '_Denominator', '_Min')
    _column_mapping = {'Label': 'Label', 'DisplayStrRef': 'DisplayStrRef', 'Denominator': 'Denominator', 'Min': 'Min'}
    _table_name = 'fractionalcr'
    _safe_columns = ['Label', 'DisplayStrRef', 'Denominator', 'Min']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<FractionalcrData {pk_column}={pk_value!r}>>"
        else:
            return f"<FractionalcrData>"
