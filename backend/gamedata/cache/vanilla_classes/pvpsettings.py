...
class PvpsettingsData:
    """
    Auto-generated data class for pvpsettings.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_label', '_value', '_strref')
    _column_mapping = {'label': 'label', 'value': 'value', 'strref': 'strref'}
    _table_name = 'pvpsettings'
    _safe_columns = ['label', 'value', 'strref']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<PvpsettingsData {pk_column}={pk_value!r}>>"
        else:
            return f"<PvpsettingsData>"
