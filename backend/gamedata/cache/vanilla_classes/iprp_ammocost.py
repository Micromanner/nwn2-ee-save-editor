...
class Iprp_ammocostData:
    """
    Auto-generated data class for iprp_ammocost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Cost', '_Arrow', '_Bolt', '_Bullet')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Cost': 'Cost', 'Arrow': 'Arrow', 'Bolt': 'Bolt', 'Bullet': 'Bullet'}
    _table_name = 'iprp_ammocost'
    _safe_columns = ['Name', 'Label', 'Cost', 'Arrow', 'Bolt', 'Bullet']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_ammocostData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_ammocostData>"
