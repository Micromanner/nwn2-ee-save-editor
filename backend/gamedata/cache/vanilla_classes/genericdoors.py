...
class GenericdoorsData:
    """
    Auto-generated data class for genericdoors.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_StrRef', '_ModelName', '_BlockSight', '_VisibleModel', '_SoundAppType', '_Name')
    _column_mapping = {'Label': 'Label', 'StrRef': 'StrRef', 'ModelName': 'ModelName', 'BlockSight': 'BlockSight', 'VisibleModel': 'VisibleModel', 'SoundAppType': 'SoundAppType', 'Name': 'Name'}
    _table_name = 'genericdoors'
    _safe_columns = ['Label', 'StrRef', 'ModelName', 'BlockSight', 'VisibleModel', 'SoundAppType', 'Name']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<GenericdoorsData {pk_column}={pk_value!r}>>"
        else:
            return f"<GenericdoorsData>"
