...
class PackspstormlordData:
    """
    Auto-generated data class for packspstormlord.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_SpellIndex', '_Label')
    _column_mapping = {'SpellIndex': 'SpellIndex', 'Label': 'Label'}
    _table_name = 'packspstormlord'
    _safe_columns = ['SpellIndex', 'Label']
    ...
    def __repr__(self):
        ...
        pk_column = 'SpellIndex'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<PackspstormlordData {pk_column}={pk_value!r}>>"
        else:
            return f"<PackspstormlordData>"
