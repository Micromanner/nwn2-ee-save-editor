...
class ArmorvisualdataData:
    """
    Auto-generated data class for armorvisualdata.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_ToolsetName', '_Prefix', '_Parent')
    _column_mapping = {'Label': 'Label', 'ToolsetName': 'ToolsetName', 'Prefix': 'Prefix', 'Parent': 'Parent'}
    _table_name = 'armorvisualdata'
    _safe_columns = ['Label', 'ToolsetName', 'Prefix', 'Parent']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ArmorvisualdataData {pk_column}={pk_value!r}>>"
        else:
            return f"<ArmorvisualdataData>"
