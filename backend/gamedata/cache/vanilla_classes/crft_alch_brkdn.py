...
class Crft_alch_brkdnData:
    """
    Auto-generated data class for crft_alch_brkdn.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_SkillRank', '_MaxItemValue')
    _column_mapping = {'SkillRank': 'SkillRank', 'MaxItemValue': 'MaxItemValue'}
    _table_name = 'crft_alch_brkdn'
    _safe_columns = ['SkillRank', 'MaxItemValue']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Crft_alch_brkdnData {pk_column}={pk_value!r}>>"
        else:
            return f"<Crft_alch_brkdnData>"
