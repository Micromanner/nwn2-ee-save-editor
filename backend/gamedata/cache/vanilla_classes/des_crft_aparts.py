...
class Des_crft_apartsData:
    """
    Auto-generated data class for des_crft_aparts.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Comment', '_NumParts', '_CraftCost', '_CraftDC')
    _column_mapping = {'Comment': 'Comment', 'NumParts': 'NumParts', 'CraftCost': 'CraftCost', 'CraftDC': 'CraftDC'}
    _table_name = 'des_crft_aparts'
    _safe_columns = ['Comment', 'NumParts', 'CraftCost', 'CraftDC']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_crft_apartsData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_crft_apartsData>"
