...
class Iprp_resistcostData:
    """
    Auto-generated data class for iprp_resistcost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_Label', '_Amount', '_Cost')
    _column_mapping = {'Name': 'Name', 'Label': 'Label', 'Amount': 'Amount', 'Cost': 'Cost'}
    _table_name = 'iprp_resistcost'
    _safe_columns = ['Name', 'Label', 'Amount', 'Cost']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_resistcostData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_resistcostData>"
