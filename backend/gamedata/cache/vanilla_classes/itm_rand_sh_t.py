...
class Itm_rand_sh_tData:
    """
    Auto-generated data class for itm_rand_sh_t.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LTE', '_col_empty_3', '_TYPE_', '_BASE_TEMPLATE')
    _column_mapping = {'LTE': 'LTE', '': 'col_empty_3', 'TYPE': 'TYPE_', 'BASE_TEMPLATE': 'BASE_TEMPLATE'}
    _table_name = 'itm_rand_sh_t'
    _safe_columns = ['LTE', 'col_empty_3', 'TYPE_', 'BASE_TEMPLATE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Itm_rand_sh_tData {pk_column}={pk_value!r}>>"
        else:
            return f"<Itm_rand_sh_tData>"
