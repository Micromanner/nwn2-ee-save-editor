...
class TreasurescaleData:
    """
    Auto-generated data class for treasurescale.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_STRREF', '_VALUE')
    _column_mapping = {'LABEL': 'LABEL', 'STRREF': 'STRREF', 'VALUE': 'VALUE'}
    _table_name = 'treasurescale'
    _safe_columns = ['LABEL', 'STRREF', 'VALUE']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<TreasurescaleData {pk_column}={pk_value!r}>>"
        else:
            return f"<TreasurescaleData>"
