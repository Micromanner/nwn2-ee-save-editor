...
class TintmapData:
    """
    Auto-generated data class for tintmap.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Variation', '_CL', '_CP', '_LE', '_CH', '_SC', '_PF', '_HD', '_NK', '_padding_5')
    _column_mapping = {'Variation': 'Variation', 'CL': 'CL', 'CP': 'CP', 'LE': 'LE', 'CH': 'CH', 'SC': 'SC', 'PF': 'PF', 'HD': 'HD', 'NK': 'NK', 'padding': 'padding_5'}
    _table_name = 'tintmap'
    _safe_columns = ['Variation', 'CL', 'CP', 'LE', 'CH', 'SC', 'PF', 'HD', 'NK', 'padding_5']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<TintmapData {pk_column}={pk_value!r}>>"
        else:
            return f"<TintmapData>"
