...
class Iprp_savingthrowData:
    """
    Auto-generated data class for iprp_savingthrow.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name', '_NameString')
    _column_mapping = {'Name': 'Name', 'NameString': 'NameString'}
    _table_name = 'iprp_savingthrow'
    _safe_columns = ['Name', 'NameString']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Iprp_savingthrowData {pk_column}={pk_value!r}>>"
        else:
            return f"<Iprp_savingthrowData>"
