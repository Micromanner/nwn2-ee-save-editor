...
class Ftext_stylesData:
    """
    Auto-generated data class for ftext_styles.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_col_empty_4', '_FontFamily', '_Style', '_BeginColor', '_EndColor', '_Duration', '_Speed')
    _column_mapping = {'Label': 'Label', '': 'col_empty_4', 'FontFamily': 'FontFamily', 'Style': 'Style', 'BeginColor': 'BeginColor', 'EndColor': 'EndColor', 'Duration': 'Duration', 'Speed': 'Speed'}
    _table_name = 'ftext_styles'
    _safe_columns = ['Label', 'col_empty_4', 'FontFamily', 'Style', 'BeginColor', 'EndColor', 'Duration', 'Speed']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Ftext_stylesData {pk_column}={pk_value!r}>>"
        else:
            return f"<Ftext_stylesData>"
