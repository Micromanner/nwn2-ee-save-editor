...
class InventorysndsData:
    """
    Auto-generated data class for inventorysnds.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_InventorySound')
    _column_mapping = {'Label': 'Label', 'InventorySound': 'InventorySound'}
    _table_name = 'inventorysnds'
    _safe_columns = ['Label', 'InventorySound']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<InventorysndsData {pk_column}={pk_value!r}>>"
        else:
            return f"<InventorysndsData>"
