...
class Des_cutconvdurData:
    """
    Auto-generated data class for des_cutconvdur.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Dialog', '_Duration')
    _column_mapping = {'Dialog': 'Dialog', 'Duration': 'Duration'}
    _table_name = 'des_cutconvdur'
    _safe_columns = ['Dialog', 'Duration']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_cutconvdurData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_cutconvdurData>"
