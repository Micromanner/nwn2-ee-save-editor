...
class LoadhintsData:
    """
    Auto-generated data class for loadhints.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_LABEL', '_HINT', '_CAMPAIGNONLY')
    _column_mapping = {'LABEL': 'LABEL', 'HINT': 'HINT', 'CAMPAIGNONLY': 'CAMPAIGNONLY'}
    _table_name = 'loadhints'
    _safe_columns = ['LABEL', 'HINT', 'CAMPAIGNONLY']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<LoadhintsData {pk_column}={pk_value!r}>>"
        else:
            return f"<LoadhintsData>"
