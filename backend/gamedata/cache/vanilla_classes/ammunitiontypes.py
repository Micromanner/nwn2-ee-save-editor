...
class AmmunitiontypesData:
    """
    Auto-generated data class for ammunitiontypes.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_label', '_Model', '_ShotSound', '_ImpactSound', '_ProjSEF', '_RotationSpd')
    _column_mapping = {'label': 'label', 'Model': 'Model', 'ShotSound': 'ShotSound', 'ImpactSound': 'ImpactSound', 'ProjSEF': 'ProjSEF', 'RotationSpd': 'RotationSpd'}
    _table_name = 'ammunitiontypes'
    _safe_columns = ['label', 'Model', 'ShotSound', 'ImpactSound', 'ProjSEF', 'RotationSpd']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<AmmunitiontypesData {pk_column}={pk_value!r}>>"
        else:
            return f"<AmmunitiontypesData>"
