...
class CraftingData:
    """
    Auto-generated data class for crafting.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_CATEGORY', '_REAGENTS', '_TAGS', '_EFFECTS', '_OUTPUT', '_SKILL', '_LEVEL')
    _column_mapping = {'CATEGORY': 'CATEGORY', 'REAGENTS': 'REAGENTS', 'TAGS': 'TAGS', 'EFFECTS': 'EFFECTS', 'OUTPUT': 'OUTPUT', 'SKILL': 'SKILL', 'LEVEL': 'LEVEL'}
    _table_name = 'crafting'
    _safe_columns = ['CATEGORY', 'REAGENTS', 'TAGS', 'EFFECTS', 'OUTPUT', 'SKILL', 'LEVEL']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<CraftingData {pk_column}={pk_value!r}>>"
        else:
            return f"<CraftingData>"
