...
class Cls_bfeat_divchaData:
    """
    Auto-generated data class for cls_bfeat_divcha.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Bonus', '_col_empty')
    _column_mapping = {'Bonus': 'Bonus', '': 'col_empty'}
    _table_name = 'cls_bfeat_divcha'
    _safe_columns = ['Bonus', 'col_empty']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Cls_bfeat_divchaData {pk_column}={pk_value!r}>>"
        else:
            return f"<Cls_bfeat_divchaData>"
