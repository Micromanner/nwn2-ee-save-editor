...
class GenderData:
    """
    Auto-generated data class for gender.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_NAME', '_GENDER', '_GRAPHIC', '_CONSTANT')
    _column_mapping = {'NAME': 'NAME', 'GENDER': 'GENDER', 'GRAPHIC': 'GRAPHIC', 'CONSTANT': 'CONSTANT'}
    _table_name = 'gender'
    _safe_columns = ['NAME', 'GENDER', 'GRAPHIC', 'CONSTANT']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<GenderData {pk_column}={pk_value!r}>>"
        else:
            return f"<GenderData>"
