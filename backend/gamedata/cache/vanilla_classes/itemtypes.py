...
class ItemtypesData:
    """
    Auto-generated data class for itemtypes.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_PropColName')
    _column_mapping = {'PropColName': 'PropColName'}
    _table_name = 'itemtypes'
    _safe_columns = ['PropColName']
    ...
    def __repr__(self):
        ...
        pk_column = 'PropColName'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<ItemtypesData {pk_column}={pk_value!r}>>"
        else:
            return f"<ItemtypesData>"
