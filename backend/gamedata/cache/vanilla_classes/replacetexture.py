...
class ReplacetextureData:
    """
    Auto-generated data class for replacetexture.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_TEXTURENAME')
    _column_mapping = {'TEXTURENAME': 'TEXTURENAME'}
    _table_name = 'replacetexture'
    _safe_columns = ['TEXTURENAME']
    ...
    def __repr__(self):
        ...
        pk_column = 'TEXTURENAME'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<ReplacetextureData {pk_column}={pk_value!r}>>"
        else:
            return f"<ReplacetextureData>"
