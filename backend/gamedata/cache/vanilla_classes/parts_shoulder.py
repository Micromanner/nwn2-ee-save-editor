...
class Parts_shoulderData:
    """
    Auto-generated data class for parts_shoulder.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_COSTMODIFIER', '_ACBONUS')
    _column_mapping = {'COSTMODIFIER': 'COSTMODIFIER', 'ACBONUS': 'ACBONUS'}
    _table_name = 'parts_shoulder'
    _safe_columns = ['COSTMODIFIER', 'ACBONUS']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Parts_shoulderData {pk_column}={pk_value!r}>>"
        else:
            return f"<Parts_shoulderData>"
