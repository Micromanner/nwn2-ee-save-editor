...
class Cls_bsplvl_sacredfistData:
    """
    Auto-generated data class for cls_bsplvl_sacredfist.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_GrantsBonusSpellcasterLevel')
    _column_mapping = {'GrantsBonusSpellcasterLevel': 'GrantsBonusSpellcasterLevel'}
    _table_name = 'cls_bsplvl_sacredfist'
    _safe_columns = ['GrantsBonusSpellcasterLevel']
    ...
    def __repr__(self):
        ...
        pk_column = 'GrantsBonusSpellcasterLevel'
        if pk_column and hasattr(self, '_' + pk_column):
            pk_value = getattr(self, '_' + pk_column)
            return f"<Cls_bsplvl_sacredfistData {pk_column}={pk_value!r}>>"
        else:
            return f"<Cls_bsplvl_sacredfistData>"
