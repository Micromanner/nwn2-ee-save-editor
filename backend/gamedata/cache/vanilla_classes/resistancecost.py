...
class ResistancecostData:
    """
    Auto-generated data class for resistancecost.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Name')
    _column_mapping = {'Name': 'Name'}
    _table_name = 'resistancecost'
    _safe_columns = ['Name']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<ResistancecostData {pk_column}={pk_value!r}>>"
        else:
            return f"<ResistancecostData>"
