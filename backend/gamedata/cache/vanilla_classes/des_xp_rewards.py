...
class Des_xp_rewardsData:
    """
    Auto-generated data class for des_xp_rewards.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_CONSTANT', '_XP', '_col_empty')
    _column_mapping = {'CONSTANT': 'CONSTANT', 'XP': 'XP', '': 'col_empty'}
    _table_name = 'des_xp_rewards'
    _safe_columns = ['CONSTANT', 'XP', 'col_empty']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_xp_rewardsData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_xp_rewardsData>"
