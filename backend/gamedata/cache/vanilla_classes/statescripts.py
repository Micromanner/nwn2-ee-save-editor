...
class StatescriptsData:
    """
    Auto-generated data class for statescripts.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_STATENAME', '_SCRIPTNAME')
    _column_mapping = {'STATENAME': 'STATENAME', 'SCRIPTNAME': 'SCRIPTNAME'}
    _table_name = 'statescripts'
    _safe_columns = ['STATENAME', 'SCRIPTNAME']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<StatescriptsData {pk_column}={pk_value!r}>>"
        else:
            return f"<StatescriptsData>"
