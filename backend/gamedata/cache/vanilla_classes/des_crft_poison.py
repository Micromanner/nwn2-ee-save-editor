...
class Des_crft_poisonData:
    """
    Auto-generated data class for des_crft_poison.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Comment', '_PoisonType', '_SaveDC', '_Duration', '_ApplyCheckDC')
    _column_mapping = {'Comment': 'Comment', 'PoisonType': 'PoisonType', 'SaveDC': 'SaveDC', 'Duration': 'Duration', 'ApplyCheckDC': 'ApplyCheckDC'}
    _table_name = 'des_crft_poison'
    _safe_columns = ['Comment', 'PoisonType', 'SaveDC', 'Duration', 'ApplyCheckDC']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Des_crft_poisonData {pk_column}={pk_value!r}>>"
        else:
            return f"<Des_crft_poisonData>"
