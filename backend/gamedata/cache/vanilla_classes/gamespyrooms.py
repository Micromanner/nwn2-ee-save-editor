...
class GamespyroomsData:
    """
    Auto-generated data class for gamespyrooms.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_REFERENCE', '_STR_REF', '_GROUP_ID')
    _column_mapping = {'REFERENCE': 'REFERENCE', 'STR_REF': 'STR_REF', 'GROUP_ID': 'GROUP_ID'}
    _table_name = 'gamespyrooms'
    _safe_columns = ['REFERENCE', 'STR_REF', 'GROUP_ID']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<GamespyroomsData {pk_column}={pk_value!r}>>"
        else:
            return f"<GamespyroomsData>"
