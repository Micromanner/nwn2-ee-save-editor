...
class Container_preferenceData:
    """
    Auto-generated data class for container_preference.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Label', '_Name')
    _column_mapping = {'Label': 'Label', 'Name': 'Name'}
    _table_name = 'container_preference'
    _safe_columns = ['Label', 'Name']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<Container_preferenceData {pk_column}={pk_value!r}>>"
        else:
            return f"<Container_preferenceData>"
