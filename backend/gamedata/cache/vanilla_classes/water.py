...
class WaterData:
    """
    Auto-generated data class for water.2da
    
    This class provides efficient attribute access to 2DA row data
    with automatic column name mapping for mod compatibility.
    """
    __slots__ = ('_resource_manager', '_Water', '_STR_REF')
    _column_mapping = {'Water': 'Water', 'STR_REF': 'STR_REF'}
    _table_name = 'water'
    _safe_columns = ['Water', 'STR_REF']
    ...
    def __repr__(self):
        ...
        if pk_column and hasattr(self, '_' + pk_column):
            ...
            return f"<WaterData {pk_column}={pk_value!r}>>"
        else:
            return f"<WaterData>"
